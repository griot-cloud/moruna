...
mod tests {
    ...
    fn tmp(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("amoru-bench-dataset-{name}"));
    ...
    fn writing_into_an_impossible_directory_is_an_error_not_a_panic() {
        let dir = std::env::temp_dir().join("amoru-bench-dataset-file/out");
        let file = std::env::temp_dir().join("amoru-bench-dataset-file");
