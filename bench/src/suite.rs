...
pub fn datasets(scale: Scale) -> Result<Vec<Dataset>> {
    Ok(vec![
        ...
        // the weights that fit `numeric-embed`, which carries two i64 and
        // sixteen f64 and therefore eighteen numeric columns. `embed-weights`
        // above is [128, 64] and does not fit it; adding this rather than
        // reshaping that one keeps every dataset already generated byte
        // identical. The dataset set is recorded in `bench/README.md` and a
        // change to it is a change to that file (preamble 6.5, PM 2026-09-22).
        Dataset {
            name: "embed-weights-numeric".to_string(),
            kind: DatasetKind::SafeTensors(vec![
                tensor("weight", DType::F32, vec![18, 64])?,
                tensor("bias", DType::F32, vec![64])?,
            ]),
        },
