...
| 9. The surface a user touches | `amoru-py`, `python/amoru` | merged (97.8%). A user's first program, from a wheel, 200,000 rows with a Python kernel appending a column inside a 512 MiB budget, completes in 11.6 s including writing and reading the data in Python. It also exceeds its budget by 13 percent, which is F4.11 and the last blocker |
