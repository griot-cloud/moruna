"""PY-T13 configuration_clamping, through `amoru.run`'s own arguments (PY-I10, f.3).
...
(`crates/amoru-py/src/translate.rs`); what is asserted here is that the note reaches the report.
"""
...
def test_py_t13_sink_sizes_are_clamped(
    ...
    report = amoru.run(
        amoru.ParquetSource(src_url),
        identity,
        amoru.ParquetSink(out_url, row_group_bytes="1MiB", file_bytes="1MiB"),
        **staging,
    )
...
def test_py_t13_an_in_range_value_is_not_clamped(
    ...
    report = amoru.run(
        amoru.ParquetSource(src_url),
        identity,
        amoru.ParquetSink(out_url, row_group_bytes="32MiB", file_bytes="64MiB"),
