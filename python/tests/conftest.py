...
# test. `AMORU_BUDGET` is the platform-owned spelling of the ceiling (preamble section 5), so
...
os.environ.setdefault("AMORU_BUDGET", "6GiB")

# `profiles.dir` defaults to `~/.amoru/profiles`, which is one fixed path shared by every run on
...
_HOME = pathlib.Path(tempfile.gettempdir()) / f"amoru-home-{os.getpid()}"
(_HOME / ".amoru").mkdir(parents=True, exist_ok=True)
...
def scratch(request: pytest.FixtureRequest) -> pathlib.Path:
    ...
    return pathlib.Path(tempfile.mkdtemp(prefix=f"amoru-{name}-{os.getpid()}-"))
...
def small_sink():
    ...
    def build(url: str, **kwargs: object):
        ...
        return amoru.ParquetSink(url, **kwargs)
