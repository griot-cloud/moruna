...
case "${1:---list}" in
  ...
  --merged)
    ...
    git worktree list --porcelain | awk '/^worktree /{w=$2} /^branch /{print w, $2}' | while read -r w b; do
      [ "$w" = "$(git rev-parse --show-toplevel)" ] && continue
      br="${b#refs/heads/}"
      # A branch with no commits of its own is an ancestor of main trivially, which
      # is "not started" and not "merged". The two are told apart by the merge
      # commit: the PM merges with "Merge <branch>" in the subject, so main records
      # which branches actually landed.
      own="$(git rev-list --count "$(git merge-base main "$br")".."$br" 2>/dev/null || echo 0)"
      landed="$(git log main --merges --format=%s | grep -cF "Merge $br" || true)"
      if [ "$own" = "0" ] && [ "$landed" = "0" ]; then
