...
// `AmoruError` is a large enum on purpose (it carries a diagnostic), and the source and sink
...
use amoru_kernel::{
    AmoruError, CancelToken, Fingerprint, InitCtx, Kernel, KernelKind, KernelState, NoState,
...
impl Kernel for Appender {
    fn fingerprint(&self) -> Fingerprint {
        Fingerprint::compute("amoru::examples::append_column", b"v1")
    }
...
fn main() {
    let dir = std::env::temp_dir().join(format!("amoru-example-{}", std::process::id()));
