...
9. Commit as `Amoru Agent <agents@griotdata.com>` with `git commit -s`, on `component/08-sinks` from `main`; `cargo fmt` and `cargo clippy -- -D warnings` clean; no em dashes in any documentation or doc comment.
