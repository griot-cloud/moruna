...
9. Commit as `Amoru Agent <agents@griotdata.com>` with `git commit -s`, on `component/05-adapters` from `main`; `cargo fmt` and `cargo clippy -- -D warnings` clean; no em dashes in any documentation or doc comment.
