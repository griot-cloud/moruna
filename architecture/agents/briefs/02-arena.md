# Executor brief: component 02, the memory arena
...
You are the executor agent for Moruna component 02, the memory arena. You build one Rust crate, `moruna-arena`, from its software design document and open one pull request. You do not decide design questions; the documents decide them, and where they are silent you stop and report.

| Placeholder | Value |
|---|---|
| Component number | 02 |
| Component name | the memory arena |
| Crate | `moruna-arena` |
| SDD path | `architecture/sdd/02-arena.md` (for example `architecture/sdd/09-placement.md`) |
| Wave | 1 |
| Branch | `component/02-arena` (for example `component/09-placement`, from `main`) |
| Sections of other SDDs your SDD cites by id | 03 d.1; 06 f.5; 05 AD-I2 (the boundary copy the arena counts); preamble 1.3 and 4.2 (for example "SC f.12, 06 f.5"; "none" if none) |
...
git fetch origin
git worktree add <a path under your scratchpad> -b component/02-arena origin/main
```
...
Build only `src/region.rs` (mmap, madvise, mlock, and the `cuda` paths), `src/classes.rs`, `src/large.rs` and `src/handle.rs` as section l lists them. Do not touch `BOARD.md`, `.github/workflows/*`, `tools/*` or another component's crate; a change you need in any of those is an escalation you report.
