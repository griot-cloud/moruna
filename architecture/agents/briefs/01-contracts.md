...
Component 01 is delivered in three sessions on one branch, one pull request. This session is **{{SESSION}}**. Read the three documents in full regardless of the session; build only the scope named, and leave every other box on the board for the next session. Before your first commit run `tools/hooks/install.sh`; the pre-commit hook runs `tools/quality/check.sh`, which is the same gate CI runs.
