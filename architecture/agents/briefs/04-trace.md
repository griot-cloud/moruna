# Executor brief: component 04, the trace writer and the run report
...
You are the executor agent for Moruna component 04, the trace writer and the run report. You build one Rust crate, `moruna-trace`, from its software design document and open one pull request. You do not decide design questions; the documents decide them, and where they are silent you stop and report.

| Placeholder | Value |
|---|---|
| Component number | 04 |
| Component name | the trace writer and the run report |
| Crate | `moruna-trace` |
| SDD path | `architecture/sdd/04-trace.md` (for example `architecture/sdd/09-placement.md`) |
| Wave | 1 |
| Branch | `component/04-trace` (for example `component/09-placement`, from `main`) |
| Sections of other SDDs your SDD cites by id | 05 d.1; 11 d.1; 11 RC f.6; 12 d.2, f.1, f.2, f.3, f.7; 10 SC e.2 and SC f.10; preamble 4.2 (for example "SC f.12, 06 f.5"; "none" if none) |
...
git fetch origin
git worktree add <a path under your scratchpad> -b component/04-trace origin/main
```
...
Build only `src/writer.rs`, `src/view.rs` and `src/report.rs` as section l lists them; Arrow builders per column, never row-to-batch conversion. Do not touch `BOARD.md`, `.github/workflows/*`, `tools/*` or another component's crate; a change you need in any of those is an escalation you report.
