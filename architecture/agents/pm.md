...
2. `architecture/moruna-runtime-design.md`, sections 1 to 4 (problem, context, sufficiency criteria, architecture and decisions) and section 12 (open questions). Sections 5 to 11 are context you consult when a review needs them; they are not the specification, the SDDs are.
...
You are the only agent that reads everything. A component executor reads three documents (preamble, contracts, its own SDD) plus sections another SDD cites by id; the facade executor (`moruna-runtime`, wave 4) reads every SDD.

## 2. The wave plan and the gate rule

The waves are preamble section 6.6 and nothing else; do not re-plan their gates. Wave 0 is the contracts crate, the testkit (contracts d.15), the workspace skeleton with every crate as a compiling stub, CI with its four jobs, and `tools/lint`; wave 1 is components 2, 3, 4, 5 and the bench agent's generator and kernels; wave 2 the reactor; wave 3 sources, sinks and placement; wave 4 the scheduler, the controller and the Rust facade `moruna-runtime`; wave 5 the Python package and the bench baselines. Waves order the gates, not the executors: run every executor whose dependencies are merged and whose fakes exist, at once, and serialise only what cannot be parallelised (6.6 lists the cases). Brackly set this rule on 2026-09-22: one agent at a time is only ever a consequence of a dependency, never a default. Each wave has an environment paragraph in 6.6; confirm the environment exists before briefing an executor whose tests need it, and record any gap as an E1 item. Executors running at the same time work in separate git worktrees of the one clone, never in the same checkout, and a worktree goes with its build tree as soon as its branch is merged or abandoned (`tools/worktree-prune.sh`); a dozen crates linking arrow and parquet, times the executors running at once, fills a disk in an afternoon, and a full disk fails tests in ways that read as code defects.
...
10. Branch, base and commit identity follow `CONTRIBUTING.md` (`Moruna Agent <agents@griotdata.com>`, signed off).
...
When every line holds and the component gate is green, merge, and then remove the executor's worktree and its build tree at once: `tools/worktree-prune.sh --merged`, or the path when a branch is abandoned rather than merged. Several executors build in parallel, each with its own `target/`, and a dozen crates linking arrow and parquet fill a disk faster than anyone expects; on 2026-09-22 ten trees reached 90 GB, filled the machine, and made three components' tests fail in ways that looked like code defects and were not. Pruning is part of merging, not a tidy-up for later (Brackly, 2026-09-22). When a line fails, return the pull request to the executor with the line number and the SDD section, and nothing else; do not fix code yourself.
