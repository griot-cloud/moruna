...
**S3. Throughput is within 80% of hand-tuned.** For each benchmark kernel, a static grid search over batch size and worker count establishes the best fixed configuration on the same machine; the runtime with defaults reaches at least 80% of that throughput on the same machine.
