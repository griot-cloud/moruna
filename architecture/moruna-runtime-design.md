...
The runtime is named Moruna (MORsel RUNtime, Adaptive). "Morsel" is the unit of work throughout this document, not the product.
...
**S7. A kernel is portable without change.** A kernel crate written against `moruna-kernel` alone compiles into the runtime, into a Polars expression plugin, and into a DataFusion scalar function through three thin wrapper crates that contain no kernel logic.
...
The contract with the transformation, and the crate boundary that makes portability possible. `moruna-kernel` contains these types and depends only on arrow.
...
**Portability.** A kernel author implements `Kernel` and nothing else. `moruna-polars` wraps a `Kernel` into a Polars expression plugin by converting the plugin's input Series to a RecordBatch and back; `moruna-datafusion` does the same for a `ScalarUDF`. Both wrappers are under a hundred lines and contain no kernel logic (S7). The runtime is the only host that uses `kind`, `hints` and `init` fully; the others treat every kernel as stateless and call `init` once.
...
Superseded by `sdd/10-scheduler.md` f.4 for instance creation (instances are created eagerly by `init_instances` before the baseline sample; the sketch below is the revision 3 text).
...
Superseded by `sdd/01-contracts.md` section e.5 and d.13 for the record schema (the schema below is the revision 3 sketch; the contracts add `instance`, `state_bytes`, `placement_miss_wait_us` and the `Probe` outcome, and the contracts' field list is the one hashed). Every morsel completion writes one record to the trace, an Arrow IPC stream on the IO reactor, flushed at run end and on failure:
...
@moruna.kernel(stateful=True, instances=1, device_memory=True)
class Score:
    def setup(self):
...
report = moruna.run(
    source=moruna.ParquetSource("s3://bucket/data/", columns=["id", "text"]),
    kernels=[normalize, Score()],
    sink=moruna.ParquetSink("s3://bucket/scored/"),
...
A plain function is a stateless kernel. Kernels may also be Polars expressions (`moruna.polars(lambda df: ...)`, applied per morsel through the lazy API) or Rust plugins loaded by name. The package is built with maturin as version-specific wheels for CPython 3.13 and 3.14, GIL and free-threaded (abi3 cannot target free-threaded builds). `moruna.run` refuses to start if a kernel is Python and the interpreter reports the GIL enabled, unless `allow_gil=True`, in which case it proceeds serialised and the report says so.
...
**Inside another engine.** `moruna-polars` and `moruna-datafusion` host a kernel, not the runtime. In those hosts the engine schedules, the engine sizes, and the kernel is a stateless batch function; the runtime's controller is absent. This is by design: the runtime cannot take over an engine's scheduler, and the kernel crate is what carries across.

**Crate layout** that makes this true: `moruna-kernel` (traits, Morsel, Payload, features; depends on arrow and dlpark only), `moruna-runtime` (everything in section 5 except 5.3's trait and 5.10), `moruna-py` (PyO3 bindings, depends on runtime), `moruna-polars` and `moruna-datafusion` (bridges, depend on kernel only). A kernel author's crate depends on `moruna-kernel` alone. The preamble's crate layout (`sdd/00-preamble.md` section 6.1) follows this: the runtime is split into one crate per component with `moruna-runtime` as the facade over them, and the two bridges are separate thin crates, not features of the adapters crate.
...
**A single row larger than the maximum morsel.** The source cannot sub-split below one row. The morsel is passed at its natural size, the controller drops active workers to what the budget allows for that morsel (possibly one), and the trace marks it. A single row larger than the budget terminates the run with a diagnostic; no runtime can process it. Tests: SO-T14 (the source delivers the oversized row as one morsel and reports it) and SC-T17 (the scheduler runs it alone and the trace marks it).
...
**A kernel that never returns.** The runtime cannot preempt `apply`. The scheduler keeps a heartbeat per worker (the task it is on and when it started); a worker past the bound is named in the run report and, on cancellation, in the diagnostic, so a hung kernel is diagnosed rather than silent. Test: SC-T19.

**Stateful `init` fails.** The run terminates before any morsel is read, with the error; nothing has been written. Every instance is initialised eagerly before the baseline sample and the probes, so the failure surfaces in `init_instances`, before the sink has received anything. Test: SC-T18.
...
**Spill directory on the same device as the source.** Not detected automatically. Documented as a configuration hazard; the report shows spill bandwidth and source bandwidth side by side so a collision is visible. Test: TR-T10 (the report's `staging_bandwidth` and `source_bandwidth` are computed from the trace and both appear).
...
**Kernel-internal allocations are governed indirectly, and the model says so.** The budget is enforced directly for every byte the runtime allocates: morsels, queues, staging, the arena. A byte a kernel allocates for itself inside `apply` (a NumPy temporary, a Torch tensor, a Rust `Vec`) is outside the arena and is seen only by the sampler, after the fact. It is nevertheless *in the model*: the controller's working set is two inequalities (11 f.3), and the second bounds the process's peak anonymous memory (the quantity S1 measures) against the ceiling, fitted to `TraceRecord::mem_anon_peak`. `budget.reserve_fraction` is the numerator of that inequality rather than a margin the design hopes covers the case. The guarantee for those bytes is still indirect, because the only levers are morsel size and worker count and both act on the next morsel rather than this one: the probe measures what a kernel does to the process, the controller sizes morsels and workers from it with a safety margin, breach handling refits and shrinks at half the headroom, and the cgroup is the containment if all of that is wrong at once. Where shrinking to `morsel.min_bytes` on one worker still does not satisfy the inequality, the run is terminated with a diagnostic naming the morsel, its footprint and the budget. It holds for any kernel whose footprint is a stable function of its input, which is the class the runtime targets, and it degrades to "diagnosed and terminated" for a kernel whose footprint is not, never to a run that finishes over budget. Closing it fully means mediating the allocators those libraries use (NumPy and PyTorch both allow the allocator to be replaced; adapters AD-O1, escalation E13), which is a change underneath the kernel author's code and is deferred to its own design. The contract's `Allocator` is the seam; nothing in v1 precludes it.

*This was not true until 2026-09-23.* Until then the model had only the arena inequality, and because the arena's pages are all touched at `new` (02 f.1), charging a kernel's out-of-arena bytes against the arena's capacity permitted a process peak of `baseline + arena_bytes + (arena_bytes − state) / 2`, which is over the ceiling by construction. The first program anyone would write (200,000 rows, a Python kernel building a list of 20,000 strings and a pyarrow array from it, a 512 MiB budget) completed at 1.09–1.11 × its ceiling. The trace showed the controller had not been blind: it breached on every one of the ten records and shed a worker each time. It simply had no term that bounded the number the criterion measures.

**A kernel's out-of-arena allowance is `budget.reserve_fraction` of the ceiling, and only that.** The facade sizes the arena at `ceiling − baseline − reserve − declared kernel state` and the arena's pages are resident from `new`, so the process rests at `ceiling − reserve` however large the budget is, and the whole allowance for what kernels allocate outside the arena is the 10% reserve, shared with the runtime's own non-arena working memory: a Parquet reader's buffers, a writer's row-group buffers, allocator retention, which on a measured macOS host take about 20 MiB of the 51 MiB a 512 MiB budget leaves. The consequence is arithmetic and not a bug: a Python kernel needs a budget several times larger than its data for the 10% to be enough, and at a budget where it is not, the honest answer is the diagnostic. Accepted position for v1: the inequality bounds the process and the diagnostic explains the refusal, which is what S1 and G-I8 ask for. Making the split between the arena and the kernels depend on what the kernels are, a smaller arena share where a Python or framework kernel is in the chain, is a change to the facade's arena sizing (12 f.1), not to the controller, and is the next thing to do about it.
...
Deliverables: `moruna-kernel` with `Kernel`, `Morsel`, `Payload` (both variants, `Tier` with only `Host` exercised), `MorselFeatures`, `KernelKind`, `KernelHints`, `PayloadSpec`; `moruna-runtime` with the host arena (unpinned, huge pages where offered), `ParquetSource` (plan from footers, read with projection and row selection into arena buffers), `ParquetSink`, a single-threaded driver that runs source → kernels → sink with fixed 128 MB morsels; the trace writer with the 5.9 schema; the benchmark generator and the identity kernel. Gate: S12 (overhead within 5% of a plain loop), S9 partial (trace written with full schema), S7 partial (`moruna-kernel` depends on arrow and dlpark only; verified by `cargo tree`).
...
Deliverables: `moruna-polars` expression plugin wrapper; `moruna-datafusion` ScalarUDF wrapper with `MorunaMemoryPool` (adapters AD-O2) so a bridged DataFusion kernel accounts against the budget; `VortexSource` (sources SO-O1), with the `vortex` crate entering the dependency table in that pull request; the normalise kernel built and run in all three hosts from one crate. Gate: S7; the Vortex source passes the Parquet source's tests over the same generated data.
...
**Q1. Name.** Decided 2026-09-15: Moruna (MORsel RUNtime, Adaptive). Crates are `moruna-kernel`, `moruna-runtime`, `moruna-py`, `moruna-polars`, `moruna-datafusion`; the Python package is `moruna`. "Morsel" remains the unit of work. Remaining checks before publishing: PyPI, GitHub namespace, trademark, domain.
...
**Q8. Hardware baseline for the direct paths.** Decided 2026-09-22: the Griot bare-metal server in Nairobi is the reference host, and no GPU host is named, so the GPUDirect Storage gates in Phases 5 and 9 stay open with their tests listed as skipped rather than assumed. The reference host is only where figures are recorded so they compare between runs. The runtime is not built for it and must not be tuned to it: discovery reads the budget and the cores from whatever host it is on, the controller measures each kernel's amplification there, and every criterion is a ratio against a baseline measured on that same host, so a larger machine raises both sides and yields more throughput with no change to the code, the defaults or the stored profiles.
