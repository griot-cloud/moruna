...
Design documents for Moruna. Code is written from these documents; a change in behaviour is a change here first.
...
| [sdd/05-adapters.md](sdd/05-adapters.md) | Component 5: kernel adapters (the Python adapter; the Polars and DataFusion bridges are separate crates, `moruna-polars` and `moruna-datafusion`) | Draft |
...
1. Contracts crate (`moruna-kernel`): types, traits, interfaces, errors, trace schema. Depends on nothing internal. Designed and frozen first.
...
12. Python surface and the Rust facade `moruna-runtime`. Depends on all.
