...
moruna/
  ...
  crates/
    ...
    moruna-kernels/           standard kernels by arguments (15 e.6); depends on moruna-kernel, serde_json, sha2, blake3 (F8.3)
...
| `sha2` | 5, `moruna-kernels` | the Python and standard kernel fingerprints of MH 4.9 (`sha256`), `moruna.std.hash` | 0.11.0, already in the lock through `datafusion` and `polars`; added by F8.3 as an E2 table addition for the PM |
...
One interpreter, not a matrix: CPython 3.14 built free-threaded (3.14t). Free-threading is what S8 is about, 3.14 is the version where that build became officially supported, and the GIL path is the same interpreter with `PYTHON_GIL=1`, which the Python stage asserts, so a second interpreter would prove nothing a flag does not. The 3.13 builds are not carried: 3.13's free-threaded build was the experimental one, pyarrow publishes no cp313t wheel, and building it from source pulls libcst, whose own PyO3 refuses a free-threaded interpreter below 3.14, which cost thirteen minutes of compiling and a red job before this was cut. Wheels are built for CPython 3.14 in both builds, because PyO3 cannot use the stable ABI for a free-threaded build (6.2) and the two are therefore separate wheels from one source; which further interpreters a release supports is a question for F5.2, answered by who asks for one, not by a matrix that grows on its own (decided by Brackly and the PM, 2026-09-22). Wave 1: the same, plus the four Python interpreters with NumPy and pyarrow installed for component 5 and the bench kernels, and a cgroup v2 host (a container is enough) for component 3's tests. Wave 2: the same, plus a filesystem that accepts `O_DIRECT` (ext4 or xfs on a local disk, not tmpfs or overlay) and, where the host allows, io_uring; MinIO for the object-store paths; GDS and CUDA tests are tagged for the reference host (E1). Wave 3: as wave 2, plus enough local disk for the staging tests (10 GiB free) and MinIO. Wave 4: as wave 3, plus a container runtime that honours `--memory` and `--cpus`, so the kill-and-resume and budget tests run under a real ceiling. Wave 5: as wave 4, plus maturin and CPython 3.14 in both builds for the wheels, Polars and DuckDB installed for the engine baseline, and the reference hardware named by E1 for the timing gates.

Before closing a wave gate whose tests need a real ceiling or an object store, the PM runs the `slow` job from the Actions tab, because those tests do not run on an ordinary push. A budget is measured against the memory the run's own cgroup already holds, which is the run itself in a pod and the whole machine on a shared runner. So a test that asserts a budget can only mean what it says where the cgroup belongs to the test: on a GitHub Actions runner the agent, Docker and the rest of that cgroup held 5.2 GB before the test started, and a 2 GiB budget genuinely could not be honoured, which the runtime said plainly and correctly. Such a test skips with the runtime's own diagnostic rather than failing or being loosened until it passes anywhere, and it closes for real in the container stage, where `--memory` makes the cgroup the run's (decided 2026-09-23, after three tests in a row were found asserting the machine they were written on rather than the design).
