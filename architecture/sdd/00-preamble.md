...
Wave 0 deliverables, all by agent 1: the Cargo workspace root; every member crate of section 6.1 as a compiling stub with its `Cargo.toml` and the pinned versions of section 6.2; `crates/amoru-testkit` per contracts d.15; `.github/workflows/ci.yml`, one pipeline whose stages are chained so a later stage runs only if the earlier ones passed: the gate (`tools/quality/check.sh`, which is the pre-commit hook's own script, plus sign-off, `cargo deny` and the lockfile check), then the container stage with `--memory` and `--cpus` and MinIO beside it for the object-store paths, the Python stage on one interpreter, and the documentation stage; `tools/lint/no_tier_wildcard.sh`; the first CI job running `tools/quality/check.sh` (already in the repository, with `tools/hooks`), so that CI and the pre-commit hook apply one gate; and `git init` with `main` as the default branch if the repository is not yet initialised.
