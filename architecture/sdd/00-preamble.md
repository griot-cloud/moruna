...
Environment per wave. Wave 0: stable Rust with the 2024 edition (the version is pinned in `rust-toolchain.toml` in this wave), `cargo`, docker for the container job, MinIO as a container, and the four Python interpreters only to prove the matrix job runs; no GPU. Wave 1: the same, plus the four Python interpreters with NumPy and pyarrow installed for component 5 and the bench kernels, and a cgroup v2 host (a container is enough) for component 3's tests. One qualification, measured on 2026-09-22: pyarrow publishes no free-threaded 3.13 (cp313t) wheel, and building it from source pulls libcst, whose own PyO3 refuses a free-threaded interpreter below 3.14, so on 3.13t the environment is NumPy without pyarrow. The CI matrix installs wheels only, records the gap and continues, rather than compiling for thirteen minutes and failing. Component 5 and component 12 therefore prove their pyarrow paths on 3.13, 3.14 and 3.14t, and a test that needs pyarrow on 3.13t is skipped with this reason and listed; whether 3.13t stays in the wheel matrix at wave 5 is decided then, on whether a wheel exists by that day (PM, 2026-09-22). Wave 2: the same, plus a filesystem that accepts `O_DIRECT` (ext4 or xfs on a local disk, not tmpfs or overlay) and, where the host allows, io_uring; MinIO for the object-store paths; GDS and CUDA tests are tagged for the reference host (E1). Wave 3: as wave 2, plus enough local disk for the staging tests (10 GiB free) and MinIO. Wave 4: as wave 3, plus a container runtime that honours `--memory` and `--cpus`, so the kill-and-resume and budget tests run under a real ceiling. Wave 5: as wave 4, plus maturin and the four interpreters for the wheel matrix, Polars and DuckDB installed for the engine baseline, and the reference hardware named by E1 for the timing gates.
