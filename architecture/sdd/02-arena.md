...
**Preamble:** `00-preamble.md`; **Contracts:** `01-contracts.md` (d.3 `Buffer`, `Allocator`, `AllocStats`; d.12 `Limits`, `HostProfile`)
**Component location:** `crates/amoru-arena`, Rust
**Consumes:** contracts (1). Feature `cuda` adds `cudarc`. **Consumed by:** reactor (6), sources (7), sinks (8), placement (9), adapters (5), controller (11)
...
**Region.** One contiguous mapping per tier: the host region (`Host` or `PinnedHost`, never both: a host is one or the other for the run) and one device region per device.
...
**AR-I3. The region is allocated once.** After `Arena::new` returns, no further `mmap`, `cudaMalloc`, `cudaHostAlloc` or `mlock` call is made for the run; a request that cannot be served from the region fails. Rationale: the flat memory footprint (architecture 5.6a) and the pod's cgroup accounting depend on this.
...
**AR-I6. Pinning is all-or-nothing and reported.** Either the whole host region is page-locked and `Tier::PinnedHost` requests succeed, or none of it is and `Tier::PinnedHost` requests return `Alloc` with `tier: PinnedHost`; the state is fixed at `new` and exposed by `is_pinned()`. Upholds G-I7.
...
pub struct ArenaConfig {
    ...
    pub host_tier: Tier,                 // Host or PinnedHost; the arena will not construct both
    pub device_bytes: Vec<(DeviceId, u64)>,
    pub page_bytes: usize,               // from Limits
    pub huge_pages: Guarantee,           // from HostProfile
...
impl Allocator for Arena { /* d.3 of contracts */ }
```

### d.2 Consumed

`amoru_kernel::{Allocator, Buffer, AllocStats, Tier, DeviceId, Guarantee, AmoruError, ALIGNMENT}` and the `ArenaHandle` trait from `buffer.rs`:
...
**f.6 Registration for RDMA (feature `rdma`).** After pinning, `ibv_reg_mr` over the whole host region once; `rdma_region()` returns the lkey/rkey and address for the source to hand to a peer.
...
**Failures.** `mmap` failure: `Config` with errno. `mlock` failure under `Guarantee::Unknown`: fall back to `Host`, set `is_pinned = false`, log at warn; under `Present`: `Config`. `cuMemHostRegister` failure: same rule. Device `cuMemAlloc` failure: f.2. Double release (a bug): debug assertion in tests; in release builds the second release is ignored and counted in a `double_release` diagnostic counter surfaced in the report.
...
**AR-T6 pin_all_or_nothing.** With memlock forbidden (ulimit in test), `PinnedHost` requests fail and `is_pinned` is false; with it allowed, they succeed. AR-I6.
...
Files: `src/lib.rs`, `src/region.rs` (f.1, f.2, mmap and cuda), `src/classes.rs` (e.2, f.3, f.4), `src/large.rs` (large table and coalescing), `src/handle.rs` (`ArenaHandle` impl), `src/stats.rs`, `src/rdma.rs` (feature). `unsafe` permitted in `region.rs` (syscalls, cuda) and `classes.rs`/`large.rs` (pointer arithmetic), each with `// SAFETY:` citing AR-I1 or AR-I2.

Use `libc` for `mmap`, `madvise`, `mlock`; do not use `memmap2` here (it is for file mappings in sources). Do not use `alloc::alloc` for arena memory. The global allocator for the process is `mimalloc`, set in `amoru-runtime`, not here.
