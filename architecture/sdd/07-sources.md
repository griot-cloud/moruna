# Moruna SDD 07: Sources (`moruna-sources`)
...
// feature "python"; lives in this crate (not in moruna-adapters) so every source is
...
The scheduler calls `read` with a `RowRange` it computes from the controller's morsel target and the split's `uncompressed_bytes / rows`; the source does not size. Every constructor takes `Arc<dyn Reactor>`, never the concrete `moruna_reactor::Reactor`, so a test builds a source over the testkit's `FakeReactor` and the facade over the real one.
