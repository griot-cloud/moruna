...
**RC-I4. A breach shrinks immediately.** When a record's `mem_anon_peak` (contracts d.13; the larger of the samples before and after `apply`, SC f.2) exceeds the **breach line**, which is `baseline + arena_bytes + reserve` capped at the ceiling, or a record's `dev_mem_peak` exceeds the device budget, the affected stage's target is halved at once and `safety` for that stage is raised by 0.2 (capped at 3.0). Upholds G-I8.
...
**f.7 Breach handling (RC-I4).** The breach line is `baseline + arena_bytes + reserve`, capped at the ceiling, and not `ceiling − reserve`: f.1 sizes the arena at `ceiling − baseline − reserve − kernel state` and 02 f.1 touches every page of it at `new`, so `baseline + arena_bytes` is the process's resting anonymous memory and a line at `ceiling − reserve` sits exactly on it. Every run would breach on its first record, which is what the first end-to-end run did (PM, 2026-09-22). The reserve is the allowance for what a kernel allocates outside the arena, so a breach is that allowance being spent, the expected kernel state is the cushion between it and the ceiling, and f.6's memory rows still fire half a reserve earlier. On `on_record` with `mem_anon_peak` above the breach line or `dev_mem_peak` above the device budget: immediately (not waiting for the tick) halve that stage's target, raise its safety by 0.2, write the knob; if the stage is already at `morsel_min` and a second breach occurs, lower `active_workers` by one; if `active_workers == 1` and at `morsel_min` and a third breach occurs, `knobs.terminate(AmoruError::Budget { seq, stage, footprint: mem_anon_peak − mem_anon_before, budget, features })` (G-I8's diagnostic; the scheduler enters `Terminating`, SC f.8).
