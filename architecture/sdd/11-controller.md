...
**Anonymous headroom.** `ceiling − resting_anon`, which with the facade's arena sizing is the reserve plus the declared kernel state. It is the whole allowance for what kernels allocate outside the arena, and it is the numerator of the anonymous inequality.
...
*The anonymous inequality.* `anon_headroom = ceiling − resting_anon`, and `anon_for_kernels = anon_headroom − state`. For each stage, `anon_allowance = target × a_anon × safety`, and `Σ_stages share × anon_allowance ≤ anon_for_kernels`. With the facade's arena sizing (`ceiling − baseline − reserve − declared kernel state`) the headroom is the reserve plus that declared state, so `budget.reserve_fraction` stops being an allowance the design hopes is enough and becomes the numerator of an inequality the controller solves.

*The fit.* `a_anon` is fitted to `TraceRecord::mem_anon_peak`, which is the figure the criterion is measured from, and every record gives two readings of it: `peak_delta` (the process's growth across the morsel) and `mem_anon_peak − resting_anon` (the bytes resident at the peak, which is the reading the delta cannot give: allocator retention, a sink's buffers, anything not attributable to one morsel). Both are divided by the bytes the model believed were in flight, `share × max(target, bytes_in)`, because that is the unit the inequality multiplies back; dividing by `bytes_in` alone, as `A_k` does, reads a one megabyte row group against a four megabyte target as four times too expensive. `a_anon` is the larger of the two readings, maximised over the last 32 records and, until that window has filled, over the probe's seed as well: a maximum because the inequality has to bound a peak rather than track a mean, and a window rather than the whole run because a fit that could only rise would hold a kernel at the size it had when the probe spoke forever, and f.4's margin, which tightens only on an accepted increase, would never tighten. The floor is `MIN_AMPLIFICATION`. The breach path performs the same refit itself (f.7), because the tick thread has not seen the record yet and the point of f.7 is to be smaller by the time the next morsel is picked up.
...
1. Refit `a_anon` from the record (f.3). A host breach is the model being wrong about what the kernel costs the process, and every decision below is made on the corrected figure.
...
4. If the stage is at `morsel_min`, `active_workers` is 1, and one morsel of the floor per stage still does not satisfy the anonymous inequality, `knobs.terminate(AmoruError::Budget { seq, stage, footprint, budget, features })` with `footprint` the out-of-arena bytes that smallest set would cost and `budget` the anonymous headroom it did not fit (G-I8's diagnostic; the scheduler enters `Terminating`, SC f.8). Every knob the controller owns is at the end of its travel and no further record will change that, so the run ends here rather than finishing over the ceiling, which is a legitimate termination under S6. The two figures the message compares are the same kind of number; a per-morsel delta against an absolute line read as "footprint 835584 exceeds budget 536818484", which says nothing.
...
**Adversarial (S6).** At the midpoint `a_k` jumps 4×; the next record breaches at half the anonymous headroom, with the other half still between it and the ceiling; f.7 refits `a_anon`, halves the target, raises safety and drops the worker count to what the anonymous inequality allows; the target reaches the level the new figures allow; the run completes. If one `morsel_min` on one worker does not satisfy that inequality, the run terminates with the diagnostic naming the morsel, the footprint and the headroom it did not fit, which is the outcome for a kernel whose out-of-arena footprint is larger than `budget.reserve_fraction × ceiling` however the knobs are set.

**A Python kernel at a tight budget (measured, 2026-09-23).** 200,000 rows, 20,000 to a row group, a kernel that builds a Python list of 20,000 strings and a pyarrow array from it, on a ten-core host. At a 512 MiB ceiling the arena is 435 MiB and the process rests at 483 MiB, so 51 MiB is the whole allowance and the runtime's own reader and writer buffers take about 20 MiB of it, so the run terminates with the diagnostic on the first morsel that reaches the floor. At 2 GiB the headroom is 205 MiB, the controller settles on a worker count and morsel size that fit, and the run completes at 0.80–0.94 of the ceiling. Before f.3 had the second inequality, both budgets completed and the 512 MiB one peaked at 1.09–1.11 × its ceiling.
