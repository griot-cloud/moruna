...
**PY-T2 exception_mapping.** Through `run_with`, each `AmoruError` variant raised from a fake (`FakeSource::fail_split`, `FakeSink::fail_at`, `FakeKernel::fail_on`, `FakeAllocator::fail_next`, `FakeReactor::fail_next`, a cancelled token) arrives as the mapped class with the diagnostic dict populated; a `Terminated` outcome arrives with `.report` and `.manifest` set. PY-I2.
