...
**f.4a `delete_object(url)` and `abort_multipart(url, upload_id)`.** The two removal calls contracts d.9 added on 2026-09-22 for a sink's resume (08 f.8). Each has a submission queue of its own and takes an object permit, like every other object operation. A delete is `object_store`'s `delete`, and an object that is not there is `Ok(())`: a resume that runs twice must not fail the second time. An abort reaches the upload through the crate's `MultipartStore`, which is the same client under a second trait and the only route to an upload this process did not start (`MultipartUpload`, what `put_multipart` returns, can abort only the upload it holds and does not survive a crash); an upload the store no longer knows is `Ok(())` for the same reason. A `file://` url is a `LocalFileSystem`, which has no multipart uploads at all, so an abort on one is `Unsupported` rather than a silent success; a delete on one unlinks the file. Neither call is on the payload path, so neither counts bytes.
...
**An operation's bytes and `shutdown`.** The buffer of a read and the view of a write travel with the request into the engine's completion callback, not in the task that awaits the answer: `shutdown` drops the runtime and with it every awaiting task, and a `pread` already under way would otherwise be writing into bytes the arena had just taken back (RE-I1). The callback holds them until the engine is done with it, whoever is still waiting; a caller whose task was dropped sees `Cancelled`.
...
**A thread the host will not give.** `Reactor::new` asks for threads in one place. `tokio`'s multi-threaded builder does not return that failure: it panics with "OS can't spawn worker thread" from inside `build`, which on a loaded build machine (a low `ulimit -u`, a cgroup pid limit) would take the process down on the caller's thread. The panic is caught there and becomes `Io { op: "reactor_start" }`, so the runtime diagnoses the failure rather than being signalled by it (G-I8). A blocking-pool thread that cannot start is already a task panic, which `tokio` turns into a `JoinError` and this component turns into the operation's `Io`.
