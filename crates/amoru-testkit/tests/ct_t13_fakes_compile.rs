//! CT-T13 fakes_compile: `amoru-testkit` implements every trait in contracts d.3 to d.13 with
...
use amoru_kernel::{
    Allocator, AmoruError, BufferView, CheckpointExtras, Completion, CopyDst, CopySrc, DType,
...
fn morsel(alloc: &FakeAllocator, seq: u64, split: u32) -> Morsel {
    ...
    let tensor = amoru_kernel::ManagedTensor::from_buffer(buffer, 0, DType::I64, vec![8])
...
fn fake_allocator() {
    ...
    assert!(matches!(err, AmoruError::Alloc { .. }), "got {err}");
...
fn fake_reactor() {
    ...
    assert!(matches!(err, AmoruError::Io { .. }), "got {err}");
...
fn fake_source() {
    ...
    assert!(matches!(err, AmoruError::Source { .. }), "got {err}");
    ...
    assert!(
        matches!(err, AmoruError::Source { split: 0, .. }),
        "got {err}"
    );
}
...
fn fake_sink() {
    ...
    assert!(matches!(err, AmoruError::Sink(_)), "got {err}");
...
fn record(seq: u64, stage: u16) -> TraceRecord {
    TraceRecord {
        ...
        q_bytes_before: vec![0; amoru_kernel::TIER_COUNT],
        q_bytes_after: vec![0; amoru_kernel::TIER_COUNT],
...
fn fake_knobs() {
    ...
    knobs.terminate(AmoruError::Budget {
        seq: 3,
        stage: 1,
        footprint: 10,
        budget: 5,
        features: amoru_kernel::MorselFeatures::default(),
    });
    let diagnostic = knobs.terminated().expect("the run was terminated");
    assert!(diagnostic.contains("morsel 3"));
    knobs.terminate(AmoruError::Cancelled);
    assert!(
        knobs
            .terminated()
...
fn futures_block_on<T>(mut future: amoru_kernel::BoxFuture<'_, T>) -> T {
