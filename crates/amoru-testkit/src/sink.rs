...
use amoru_kernel::{
    AmoruError, BoxFuture, Payload, PayloadKind, PayloadSpec, Result, Seq, Sink, SinkSummary,
...
impl Sink for FakeSink {
    ...
    fn write(&self, seq: Seq, payload: Payload) -> BoxFuture<'_, Result<()>> {
        ...
        Box::pin(async move {
            ...
            if failing {
                return Err(AmoruError::Sink(format!("FakeSink::fail_at({seq})")));
            }
    ...
    fn resume(
        ...
        if !self.inner.resumable {
            return Err(AmoruError::Resume("FakeSink is not resumable".into()));
        }
