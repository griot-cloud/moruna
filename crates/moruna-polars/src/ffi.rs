//! Moving one column between Polars and Moruna through the Arrow C Data Interface.
//!
...
//!
//! 05-adapters section l permits `unsafe` in `moruna-adapters`'s `cross.rs` and nowhere else in
...
use moruna_kernel::arrow::ffi::{FFI_ArrowArray, FFI_ArrowSchema};
...
pub fn series_to_arrow(series: &Series) -> PolarsResult<ArrayRef> {
    ...
    let data = unsafe { moruna_kernel::arrow::ffi::from_ffi(array, &schema) }
...
pub fn arrow_to_series(name: PlSmallStr, array: &ArrayRef) -> PolarsResult<Series> {
    let (exported_array, exported_schema) = moruna_kernel::arrow::ffi::to_ffi(&array.to_data())
        .map_err(|e| PolarsError::ComputeError(e.to_string().into()))?;
