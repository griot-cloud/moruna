...
use amoru_kernel::arrow::array::RecordBatch;
use amoru_kernel::{Allocator, BoxFuture, Payload, Result, RowRange, Split, Tier};
use bytes::Bytes;
...
fn concat(
    schema: &amoru_kernel::arrow::datatypes::SchemaRef,
    ...
    match batches.len() {
        0 => RecordBatch::try_new_with_options(
            ...
            schema
                .fields()
                .iter()
                .map(|f| amoru_kernel::arrow::array::new_empty_array(f.data_type()))
                .collect(),
            &amoru_kernel::arrow::array::RecordBatchOptions::new().with_row_count(Some(0)),
        )
        .map_err(|e| source_err(split, format!("an empty row group: {e}"))),
        1 => Ok(batches.remove(0)),
        _ => amoru_kernel::arrow::compute::concat_batches(schema, batches.iter())
