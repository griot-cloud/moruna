//! Amoru component 7, the sources (`Source` over Parquet, safetensors, aligned binary and a
//! Python iterator).
//!
...
#![deny(missing_docs)]
#![deny(unsafe_code)]
// `AmoruError` carries a morsel's features (CT-I10), so every `Result` in the crate has a
