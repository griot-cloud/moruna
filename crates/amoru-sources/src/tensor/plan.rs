...
use amoru_kernel::{AmoruError, DType, Result, SourceSchema, Split, SplitId};
...
pub(crate) fn read_prefix(path: &std::path::Path, offset: u64, len: u64) -> Result<Vec<u8>> {
    ...
    let io = |e: std::io::Error| AmoruError::Io {
        op: "read_file",
...
pub(crate) fn detect(path: &std::path::Path, prefix: &[u8]) -> Result<Format> {
    ...
    Err(plan_err(format!(
        "{}: not a safetensors, .npy or AMB1 file (extension {extension:?}, no known magic)",
        path.display()
    )))
}
...
/// The `AMB1` header (contracts e.4).
///
/// `amb1::Header::read` validates that the buffer holds the payload too, so it cannot parse a
...
fn amb1_plan(path: &std::path::Path, len: u64, prefix: &[u8]) -> Result<Vec<Entry>> {
    ...
    if ndim > amoru_kernel::amb1::MAX_NDIM {
        return Err(bad(format!(
            "ndim {ndim} exceeds {}",
            amoru_kernel::amb1::MAX_NDIM
        )));
    }
    ...
    if rebuilt != actual {
        return Err(bad(
            "the header does not match the bytes the AMB1 writer would produce".to_string(),
        ));
    }
