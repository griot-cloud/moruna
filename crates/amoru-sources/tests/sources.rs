...
use amoru_kernel::arrow::array::{Array, Float64Array, Int64Array, StringArray};
use amoru_kernel::{
    Allocator, AmoruError, ObjectMetadata, Payload, Reactor, RowRange, Source, SourceSchema, Split,
    Tier,
};
use amoru_sources::{
...
fn table(payload: &Payload) -> &amoru_kernel::arrow::array::RecordBatch {
    match payload {
...
fn so_t1_plan_before_read() {
    ...
    assert!(
        matches!(error, AmoruError::Source { split: 99, .. }),
        "{error}"
    );
...
fn so_t3_resident_tier() {
    ...
    let (amb1, _) = write_amb1(&dir, "t3", vec![8, 4], 0.0);
    ...
    for pinned in [false, true] {
        ...
        assert!(
            matches!(error, AmoruError::Alloc { tier, .. } if tier == other),
            "{error}"
        );

        let tensor = tensor_over(&amb1, seed(FakeReactor::new(), &amb1));
        let splits = tensor.plan().expect("the plan");
...
fn so_t8_pruning() {
    ...
    let bare = dir.join("t8-nostats.parquet");
    {
        ...
        let strings: StringArray = (0..10)
            ...
            .collect::<Vec<_>>()
            .into();
        let batch = amoru_kernel::arrow::array::RecordBatch::try_new(
            schema,
...
fn so_t9_tensor_view_in_arena() {
    ...
    assert!(matches!(error, AmoruError::Source { .. }), "{error}");
...
fn so_t10_iterator_source() {
    use amoru_sources::PyIteratorSource;
    use pyo3::prelude::*;
    ...
    let (iter, schema) = pyo3::Python::attach(|py| {
        ...
        let iter = module
            ...
            .unbind();
        let schema =
            SourceSchema::Table(Arc::new(amoru_kernel::arrow::datatypes::Schema::new(vec![
                amoru_kernel::arrow::datatypes::Field::new(
                    "value",
                    amoru_kernel::arrow::datatypes::DataType::Int64,
                    true,
                ),
            ])));
        (iter, schema)
...
fn so_t11_schema_mismatch() {
    ...
    let second = dir.join("t11-b.parquet");
    {
        let schema = Arc::new(amoru_kernel::arrow::datatypes::Schema::new(vec![
            amoru_kernel::arrow::datatypes::Field::new(
                "i64_0",
                amoru_kernel::arrow::datatypes::DataType::Float64,
                false,
            ),
        ]));
        ...
        let batch =
            amoru_kernel::arrow::array::RecordBatch::try_new(schema, vec![Arc::new(values)])
                .expect("the batch");
        writer.write(&batch).expect("writing");
        writer.close().expect("closing");
    }
...
fn so_t14_oversized_row_natural_size() {
    ...
    assert!(matches!(error, AmoruError::Alloc { .. }), "{error}");
...
fn so_t15_read_failure_paths() {
    ...
    let error = block_on(source.read(&split, None, &alloc, Tier::Host)).unwrap_err();
    assert!(
        matches!(error, AmoruError::Source { split: id, .. } if id == split.id),
        "{error}"
    );
...
fn dictionary_columns_are_unpacked_before_the_copy() {
    ...
    assert!(!matches!(
        ...
        amoru_kernel::arrow::datatypes::DataType::Dictionary(_, _)
    ));
    let strings = batch
        .column(0)
        .as_any()
...
fn tensor_formats_plan_and_refuse() {
    ...
    assert!(matches!(error, AmoruError::Plan(_)), "{error}");
...
fn schemas_describe_the_plan() {
    ...
    assert_eq!(dtype, amoru_kernel::DType::F32);
...
fn tensor_err(dir: &std::path::Path, name: &str, bytes: &[u8]) -> AmoruError {
...
/// Every way an `AMB1` header can be wrong is named, not panicked on (contracts e.4, h).
#[test]
fn amb1_headers_are_validated() {
    let dir = scratch();
    let good = |dtype: u8, ndim: u8, shape: &[i64], data_offset: u64, payload: usize| {
        let mut bytes = b"AMB1".to_vec();
    ...
    for (name, bytes, wanted) in [
        ...
        ("ndim.amb1", good(10, 9, &[4], 4096, 16), "ndim 9 exceeds"),
        (
            "offset.amb1",
            good(10, 1, &[4], 4000, 16),
    ...
    // A header whose padding is not what the AMB1 writer would produce is rejected.
    let mut tampered = good(10, 1, &[4], 4096, 16);
    tampered[100] = 0xff;
    let error = tensor_err(&dir, "tampered.amb1", &tampered).to_string();
    ...
    cut.truncate(64);
    let error = tensor_err(&dir, "cut.amb1", &cut).to_string();
...
fn plan_and_read_failures_are_named() {
    ...
    assert!(matches!(error, AmoruError::Io { .. }), "{error}");
}
