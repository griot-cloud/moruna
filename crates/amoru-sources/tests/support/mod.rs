...
use amoru_kernel::arrow::array::{
    ArrayRef, Float64Array, Int64Array, RecordBatch, StringArray, StringDictionaryBuilder,
};
use amoru_kernel::arrow::datatypes::{DataType, Field, Int32Type, Schema, SchemaRef};
use amoru_kernel::{AllocStats, Allocator, Buffer, Completion, ObjectMeta, ObjectMetadata, Tier};
...
/// Write an `AMB1` tensor (contracts e.4) of `f32` values `base + i`.
pub fn write_amb1(dir: &Path, name: &str, shape: Vec<i64>, base: f32) -> (PathBuf, Vec<f32>) {
    let path = dir.join(format!("{name}.amb1"));
    let header = amoru_kernel::amb1::Header {
        dtype: amoru_kernel::DType::F32,
        shape: shape.clone(),
        data_offset: amoru_kernel::amb1::Header::data_offset_for(shape.len(), 4096)
    ...
    std::fs::write(&path, &bytes).expect("the amb1 file");
    (path, values)
}
...
pub fn seed(reactor: amoru_testkit::FakeReactor, path: &Path) -> amoru_testkit::FakeReactor {
...
pub fn seed_truncated(
    reactor: amoru_testkit::FakeReactor,
    path: &Path,
    keep: usize,
) -> amoru_testkit::FakeReactor {
...
pub fn err<T>(r: amoru_kernel::Result<T>) -> amoru_kernel::AmoruError {
    match r {
...
impl Allocator for CountingAllocator {
    fn alloc(&self, bytes: usize, tier: Tier) -> amoru_kernel::Result<Buffer> {
