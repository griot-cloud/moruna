...
pub(crate) fn plan(path: &std::path::Path, len: u64, prefix: &[u8]) -> Result<Vec<Entry>> {
    ...
    for (name, value) in object {
        ...
        let dtype = dtype_of(dtype_text).ok_or_else(|| {
            plan_err(format!(
                "{}: tensor {name} has dtype {dtype_text}, which Moruna has no DType for (contracts d.4)",
                path.display()
            ))
        })?;
        let shape = info
...
/// The `DType` a safetensors dtype string names; `None` for one Moruna has no element type for
...
mod tests {
    ...
    #[test]
    fn every_moruna_dtype_has_a_safetensors_name() {
