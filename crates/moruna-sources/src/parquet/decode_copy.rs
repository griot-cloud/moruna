...
pub(crate) fn copy_batch(
    ...
    .map_err(|e| MorunaError::Plan(format!("rebuilding the decoded batch: {e}")))?;
    Ok((batch, copied))
}

/// A dictionary column as a plain array of its value type; anything else unchanged (f.4).
fn unpack_dictionary(
    column: &moruna_kernel::arrow::array::ArrayRef,
) -> Result<moruna_kernel::arrow::array::ArrayRef> {
    match column.data_type() {
        DataType::Dictionary(_, values) => {
            moruna_kernel::arrow::compute::cast(column, values.as_ref())
                .map_err(|e| MorunaError::Plan(format!("unpacking a dictionary column: {e}")))
        }
        _ => Ok(column.clone()),
    }
}
