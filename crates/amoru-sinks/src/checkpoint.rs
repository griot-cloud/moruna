...
use amoru_kernel::{AmoruError, Seq};
...
impl Ledger {
    ...
    pub(crate) fn to_bytes(&self) -> amoru_kernel::Result<Vec<u8>> {
        let doc = Document {
            version: VERSION,
        ...
        serde_json::to_vec(&doc)
            .map_err(|e| AmoruError::Sink(format!("sink checkpoint could not be serialised: {e}")))
    }

    /// Parse e.5, refusing an unknown version or a kind that does not match this sink (f.8).
    pub(crate) fn parse(kind: &'static str, state: &[u8]) -> amoru_kernel::Result<Document> {
        let doc: Document = serde_json::from_slice(state)
            .map_err(|e| AmoruError::Resume(format!("sink checkpoint is not readable: {e}")))?;
    ...
    ) -> amoru_kernel::Result<(Ledger, Vec<CommittedFile>)> {
