...
//! nothing is copied. The AMB1 header and the safetensors header are metadata, written from
...
use amoru_kernel::{
    Allocator, AmoruError, BoxFuture, Buffer, BufferView, Completion, DType, ManagedTensor,
...
/// The kind a per-morsel AMB1 sink writes into its checkpoint (e.5).
...
pub enum TensorFormat {
    ...
    SafeTensors,
    /// The Amoru aligned binary format of contracts e.4.
    Amb1,
}
...
pub struct TensorSinkConfig {
    ...
    /// One file per morsel (AMB1 only), rather than one file for the run.
...
/// A sink that writes tensors as safetensors or AMB1 files (e.4).
...
impl TensorSink {
    ...
    /// True for the one shape of this sink that tracks commits: AMB1, one file per morsel.
    ...
    /// The page size AMB1 aligns to: the host's, and never below the 4096 e.4 fixes.
    fn page(&self) -> u64 {
    ...
    fn file_name(&self, seq: Option<Seq>) -> String {
        match (seq, self.cfg.format) {
            (Some(seq), _) => format!("{}-{seq:012}.amb1", self.cfg.name),
            (None, TensorFormat::Amb1) => format!("{}.amb1", self.cfg.name),
    ...
    fn open_run_file(&self, inner: &mut Inner) -> Result<()> {
        ...
        let data_offset = match self.cfg.format {
            TensorFormat::Amb1 => amb1::Header::data_offset_for(shape.len(), self.page())?,
    ...
    /// One AMB1 file for this morsel: the header at 0, the tensor at the page-aligned data
    ...
    fn submit_per_morsel(
        ...
        let header = amb1::Header {
            dtype: tensor.dtype(),
            shape: tensor.shape().to_vec(),
            data_offset: amb1::Header::data_offset_for(tensor.shape().len(), self.page())?,
        };
        let mut buf = self
            .alloc
...
/// The sequence number a per-morsel AMB1 file name carries (e.4).
fn seq_of(name: &str, stem: &str) -> Option<Seq> {
    let rest = name.strip_prefix(stem)?.strip_prefix('-')?;
    rest.strip_suffix(".amb1")?.parse().ok()
}
...
impl Sink for TensorSink {
    ...
    fn resume(
        ...
        if !self.is_resumable() {
            return Err(AmoruError::Resume("sink does not support resume".into()));
        }
        let SourceSchema::Tensor { dtype, shape } = schema else {
            return Err(AmoruError::Sink(
                "the tensor sink accepts a tensor schema".into(),
