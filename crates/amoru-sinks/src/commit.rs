...
use amoru_kernel::{AmoruError, BufferView, Completion, Reactor, Result};
...
impl ObjectPrefix {
    ...
    pub(crate) fn list(&self) -> Result<Vec<String>> {
        match &self.local {
            ...
            None => Err(AmoruError::Resume("cannot list destination".into())),
        }
    }
...
fn io_err(op: &'static str, path: &Path, e: &std::io::Error) -> AmoruError {
    AmoruError::Io {
        op,
