...
fn footer_range(bytes: &[u8]) -> Option<(u64, u64)> {
    ...
    for i in 0..metadata.len() {
        ...
        match (kv.key(), kv.value()) {
            (Some("amoru.seq_min"), Some(v)) => min = v.parse().ok(),
            (Some("amoru.seq_max"), Some(v)) => max = v.parse().ok(),
            _ => {}
        }
    }
    Some((min?, max?))
...
fn si_t13_resume_removes_uncommitted() {
    ...
    for seq in 0..total {
        block_on(first.write(
            seq,
            amoru_kernel::Payload::table(inputs[seq as usize].clone()).expect("payload"),
        ))
        .expect("write");
        if seq == 120 {
...
fn si_t14_unresumable_says_so() {
    ...
    for (name, state) in [
        ("amb1 per morsel", per_morsel.checkpoint()),
