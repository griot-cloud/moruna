...
use amoru_kernel::arrow::datatypes::{DataType, Field, Schema, SchemaRef};
use amoru_kernel::arrow::record_batch::RecordBatch;
use amoru_kernel::{DType, ManagedTensor, Payload, SourceSchema, Tier};
...
impl Scratch {
    pub fn new(tag: &str) -> Scratch {
        ...
        let path =
            std::env::temp_dir().join(format!("amoru-sinks-{tag}-{}-{n}", std::process::id()));
...
pub fn arena_wide_batch(alloc: &FakeAllocator, rows: usize) -> RecordBatch {
    use amoru_kernel::arrow::array::Int64Array;
...
pub fn written(reactor: &amoru_testkit::FakeReactor, dir: &Path, name: &str) -> Vec<u8> {
...
pub fn materialise(reactor: &amoru_testkit::FakeReactor, dir: &Path) {
...
impl amoru_kernel::Allocator for CountingAlloc {
    fn alloc(&self, bytes: usize, tier: Tier) -> amoru_kernel::Result<amoru_kernel::Buffer> {
        self.inner.alloc(bytes, tier)
    }

    fn page_bytes(&self) -> usize {
        amoru_kernel::Allocator::page_bytes(&self.inner)
    }

    fn stats(&self) -> amoru_kernel::AllocStats {
        amoru_kernel::AllocStats {
