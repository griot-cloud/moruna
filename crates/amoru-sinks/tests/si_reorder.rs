...
use amoru_kernel::{AmoruError, Seq, Sink, SourceSchema};
use amoru_sinks::{ReorderBuffer, SinkHandle};
use amoru_testkit::{FakeAllocator, FakeSink};
...
fn drive(buffer: &ReorderBuffer<dyn Sink>, alloc: &FakeAllocator, order: &[Seq]) -> bool {
    ...
    type Pending<'a> = Pin<Box<dyn Future<Output = amoru_kernel::Result<()>> + Send + 'a>>;
...
fn a_sequence_out_of_range_is_refused() {
    ...
    assert!(matches!(refused, Err(AmoruError::Sink(_))), "{refused:?}");
...
fn si_t15_sink_handle() {
    ...
    assert!(matches!(refused, Err(AmoruError::Resume(_))), "{refused:?}");
