...
use amoru_kernel::{
    Allocator, AmoruError, CancelToken, Device, DeviceId, Guarantee, HostProfile, Kernel,
    LimitSource, Limits, Placement, Reactor, RunId, Sampler, Sink, Source, TierKind, TraceSink,
    TraceTail,
};
use amoru_runtime::{Components, RunSpec, Runtime, SinkSpec, SourceSpec};
use amoru_testkit::{
...
/// PY-T2: a kernel error arrives as `AmoruError::Kernel` with the partial report attached.
#[test]
fn py_t2_kernel_error_is_mapped() {
    ...
    assert!(
        matches!(error.error, AmoruError::Kernel { .. }),
...
fn py_t6_cancel_before_run() {
    ...
    assert!(
        matches!(error.error, AmoruError::Cancelled),
...
fn py_t12_resume_refuses_a_one_shot_source() {
    ...
    assert!(
        matches!(error.error, AmoruError::Resume(_)),
...
fn rt_t4_inspect_reads_the_host() {
    let discovered = Runtime::inspect(&amoru_discovery::DiscoveryInput {
