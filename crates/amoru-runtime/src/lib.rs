//! Amoru facade, the Rust side of component 12: wires components 2 to 11 into
...
#![deny(missing_docs)]
// `AmoruError` is the contracts crate's error and is 128 bytes wide; every crate in the
...
pub use amoru_discovery::{Discovered, DiscoveryInput};
pub use amoru_kernel::{CancelToken, ErrorPolicy, RunId, SizerKind};
pub use amoru_trace::{ExitReason, RunReport};

/// The process allocator for everything outside the arena (12 l). It is set here rather than
/// in `amoru-py` because the Python module links this crate, so one setting covers both.
#[global_allocator]
