...
use amoru_discovery::Discovered;
use amoru_kernel::{
...
use amoru_reactor::ObjectStoreConfig;
...
impl BuildCtx {
    ...
    pub fn object_metadata(&self) -> amoru_kernel::Result<Arc<dyn ObjectMetadata>> {
        match &self.object_metadata {
            Some(meta) => Ok(meta.clone()),
            None => Err(amoru_kernel::AmoruError::Config {
...
pub type SourceFactory = Box<dyn FnOnce(&BuildCtx) -> amoru_kernel::Result<Arc<dyn Source>> + Send>;
/// A sink builder, run at the lifecycle's "sinks built" step.
pub type SinkFactory = Box<dyn FnOnce(&BuildCtx) -> amoru_kernel::Result<Box<dyn Sink>> + Send>;
...
impl SourceSpec {
    ...
    pub fn build(self, ctx: &BuildCtx) -> amoru_kernel::Result<Arc<dyn Source>> {
        match self {
...
impl SinkSpec {
    /// Run the builder, or hand back what was already built.
    pub fn build(self, ctx: &BuildCtx) -> amoru_kernel::Result<Box<dyn Sink>> {
        match self {
...
pub struct RunSpec {
    ...
    #[cfg(feature = "python")]
    pub py_kernels: Vec<(amoru_kernel::StageId, Arc<amoru_adapters::PyKernel>)>,
...
impl RunSpec {
    ...
    /// default (section 5). This is what `amoru.run(source, kernels, sink)` translates to
...
mod tests {
    use super::*;
    use amoru_testkit::{FakeAllocator, FakeReactor, FakeSink, FakeSource};

    fn ctx(metadata: Option<Arc<dyn ObjectMetadata>>) -> BuildCtx {
        BuildCtx {
            discovered: Discovered {
                limits: amoru_kernel::Limits {
                    memory_ceiling: 1 << 30,
                    memory_kill: None,
                    cpu_quota: 1.0,
                    page_bytes: 4096,
