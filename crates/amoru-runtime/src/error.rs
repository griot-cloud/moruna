...
//! Python exception. `AmoruError` has nowhere to put either, so the facade's `Result` carries
//! this type; the underlying `AmoruError` is `error` and maps to the Python class as e.2 says.
//!
...
impl RunError {
    ...
    pub fn bare(error: AmoruError) -> RunError {
...
impl From<AmoruError> for RunError {
    fn from(error: AmoruError) -> RunError {
        RunError::bare(error)
    }
}
...
mod tests {
    ...
    fn a_failure_carries_its_parts() {
        let error = RunError::bare(AmoruError::Cancelled)
            .with_manifest(Some(PathBuf::from("/tmp/amoru/manifest.json")))
        ...
        assert!(matches!(parts.error, AmoruError::Cancelled));
    ...
    fn a_bare_failure_is_just_the_diagnostic() {
        let error: RunError = AmoruError::Plan("no splits".into()).into();
        assert!(!error.is_resumable());
        assert_eq!(
            error.to_string(),
            AmoruError::Plan("no splits".into()).to_string()
        );
