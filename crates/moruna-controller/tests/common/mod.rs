...
pub fn high_waters(writes: &[Knob]) -> Vec<(StageId, moruna_kernel::TierKind, u64)> {
    writes
        .iter()
        .filter_map(|knob| match knob {
...
impl Prober for RecordingProber {
    fn probe(&self, stage: StageId, bytes: u64) -> Result<ProbeResult> {
        ...
        if let Some(msg) = &self.fails_with {
            return Err(moruna_kernel::MorunaError::Plan(msg.clone()));
        }
...
impl Knobs for Watcher<FakeKnobs> {
    ...
    fn terminate(&self, diagnostic: moruna_kernel::MorunaError) {
...
impl Placement for WatchedPlacement {
    fn push(&self, _stage: StageId, _morsel: moruna_kernel::Morsel) -> Result<()> {
        self.other();
        Ok(())
    }
    ...
    fn pop(
        &self,
        _stage: StageId,
        _want: moruna_kernel::PayloadSpec,
        _locality: moruna_kernel::Locality,
    ) -> Result<Option<moruna_kernel::Morsel>> {
        self.other();
        Ok(None)
    }
    ...
    fn pop_blocking(
        ...
        _want: moruna_kernel::PayloadSpec,
        _locality: moruna_kernel::Locality,
    ) -> Result<Option<(moruna_kernel::Morsel, u64)>> {
        self.other();
        Ok(None)
    }
    ...
    fn evicted(&self, _stage: StageId) -> Vec<(Seq, moruna_kernel::Origin)> {
        self.other();
        Vec::new()
    }

    fn replace(&self, _stage: StageId, _morsel: moruna_kernel::Morsel) -> Result<()> {
        self.other();
        Ok(())
    }
    ...
    fn checkpoint(&self, _extras: &moruna_kernel::CheckpointExtras) -> Result<PathBuf> {
        self.other();
    ...
    fn set_consumer(&self, _stage: StageId, _want: moruna_kernel::PayloadSpec) {
        self.other();
    }
    ...
    fn set_water(&self, _stage: StageId, _tier: moruna_kernel::TierKind, _low: u64, _high: u64) {
        self.other();
    }
    ...
    fn stats(&self) -> moruna_kernel::PlacementStats {
        self.other();
        moruna_kernel::PlacementStats::default()
    }
}
