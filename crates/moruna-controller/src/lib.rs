//! Moruna component 11, the resource controller: the one writer of every knob.
//!
...
/// The contracts' result type; every error here is an `MorunaError` value (contracts d.14).
pub type Result<T> = core::result::Result<T, MorunaError>;
...
pub(crate) fn features_of(r: &TraceRecord) -> moruna_kernel::MorselFeatures {
    moruna_kernel::MorselFeatures {
        rows: r.rows_in,
        bytes: r.bytes_in,
...
pub(crate) struct Actions {
    pub knobs: Vec<moruna_kernel::Knob>,
    pub budgets: Option<TierBudgets>,
    pub terminate: Option<MorunaError>,
}
