//! The `normalise` kernel as an `amoru_kernel::Kernel`, for AD-T9.
//!
...
use amoru_kernel::arrow::array::{Array, ArrayRef, StringArray, StringBuilder};
use amoru_kernel::arrow::datatypes::{DataType, Field, Schema};
use amoru_kernel::arrow::record_batch::RecordBatch;
use amoru_kernel::{
    AmoruError, Fingerprint, InitCtx, Kernel, KernelKind, KernelState, NoState, Payload,
...
impl Kernel for Normalise {
    ...
    fn output_schema(&self, input: &SourceSchema) -> Result<SourceSchema> {
        let SourceSchema::Table(schema) = input else {
            return Err(AmoruError::Plan("normalise wants a table".into()));
        };
        let Some(field) = schema.fields().first() else {
            return Err(AmoruError::Plan("normalise wants one text column".into()));
        };
    ...
    fn apply(&self, _state: &mut dyn KernelState, input: Payload) -> Result<Payload> {
        ...
        let text = amoru_kernel::arrow::compute::cast(column, &DataType::Utf8)
            .map_err(|e| AmoruError::Plan(format!("normalise: {e}")))?;
        let Some(values) = text.as_any().downcast_ref::<StringArray>() else {
            return Err(AmoruError::Plan("normalise: not a text column".into()));
        };
...
impl Kernel for TensorOut {
    fn fingerprint(&self) -> Fingerprint {
        Fingerprint::compute("amoru-test:tensor-out", b"")
    }
...
impl Kernel for NoColumns {
    fn fingerprint(&self) -> Fingerprint {
        Fingerprint::compute("amoru-test:no-columns", b"")
    }
    ...
    fn apply(&self, _state: &mut dyn KernelState, input: Payload) -> Result<Payload> {
        ...
        let batch = RecordBatch::try_new_with_options(
            ...
            vec![],
            &amoru_kernel::arrow::record_batch::RecordBatchOptions::new()
                .with_row_count(Some(rows)),
        )
        .map_err(|e| AmoruError::Plan(format!("no columns: {e}")))?;
        Payload::table(batch)
    }
}
...
impl Kernel for Failing {
    fn fingerprint(&self) -> Fingerprint {
        Fingerprint::compute("amoru-test:failing", b"")
    }
    ...
    fn apply(&self, _state: &mut dyn KernelState, _input: Payload) -> Result<Payload> {
        Err(AmoruError::Plan("this kernel always refuses".into()))
    }
}
