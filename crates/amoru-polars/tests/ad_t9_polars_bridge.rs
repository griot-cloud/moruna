...
//! and inside Amoru, on the same input, with identical output.
//!
...
use amoru_polars::polars_plugin;
...
/// The same rows, as Amoru sees them.
...
fn ad_t9_polars_bridge() {
    ...
    assert_eq!(
        inside_polars, inside_amoru,
        "the same kernel produced different output in Polars and in Amoru (S7)"
    );
}
...
fn ad_t9_polars_bridge_carries_refusals() {
    ...
    assert!(
        matches!(
            amoru_polars::run_kernel(&common::TensorOut, std::slice::from_ref(&numbers)),
    ...
    assert!(
        matches!(
            amoru_polars::run_kernel(&common::NoColumns, std::slice::from_ref(&numbers)),
            Err(PolarsError::ComputeError(_))
        ),
        "a batch with no columns has no Series to return"
    );

    let refused = amoru_polars::run_kernel(&common::Failing, std::slice::from_ref(&numbers))
