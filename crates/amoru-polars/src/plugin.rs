...
use amoru_kernel::arrow::array::ArrayRef;
use amoru_kernel::arrow::datatypes::{Field, Schema};
use amoru_kernel::arrow::record_batch::RecordBatch;
use amoru_kernel::{AmoruError, Kernel, NoState, Payload};
...
pub fn run_kernel(kernel: &dyn Kernel, inputs: &[Series]) -> PolarsResult<Series> {
    if inputs.is_empty() {
        return Err(PolarsError::ComputeError(
            "an Amoru kernel plugin needs at least one input series".into(),
        ));
    }
    ...
    let payload = Payload::table(batch).map_err(amoru)?;
    let mut state = NoState;
    let output = kernel.apply(&mut state, payload).map_err(amoru)?;
    let Payload::Table(out, _) = output else {
        return Err(PolarsError::ComputeError(
            "an Amoru kernel returned a tensor, which a Polars expression cannot carry".into(),
        ));
    };
    let Some(column) = out.columns().first() else {
        return Err(PolarsError::ComputeError(
            "an Amoru kernel returned no columns".into(),
        ));
    };
    let name = out
        .schema()
        ...
        .unwrap_or_else(|| PlSmallStr::from_static("amoru"));
    ffi::arrow_to_series(name, column)
}

fn amoru(error: AmoruError) -> PolarsError {
