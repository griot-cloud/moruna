//! Amoru component 10, the scheduler: workers, drives, the admission rule and the trace.
//!
...
use amoru_kernel::{
    Allocator, AmoruError, CancelToken, Kernel, NodeId, Placement, RecordHook, Result, ResumePoint,
    Sampler, TraceSink,
};
...
impl amoru_kernel::Knobs for Scheduler {
    fn set(&self, knob: amoru_kernel::Knob) {
...
impl amoru_kernel::StatsSource for Scheduler {
    fn scheduler_stats(&self) -> amoru_kernel::SchedulerStats {
        stats::scheduler_stats(&self.shared)
    }
}

impl amoru_kernel::Prober for Scheduler {
    fn probe(&self, stage: amoru_kernel::StageId, bytes: u64) -> Result<amoru_kernel::ProbeResult> {
