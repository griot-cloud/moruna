...
use amoru_kernel::{AmoruError, CancelToken, Knobs};
...
fn sc_t15_checkpoint_tick() {
    ...
    let outcome = std::thread::scope(|scope| {
        ...
        rig.scheduler.terminate(AmoruError::Budget {
            seq: 0,
            stage: 1,
            footprint: 2,
            budget: 1,
    ...
    assert!(
        threads.contains(&"amoru-checkpoint".to_string()),
        "the ticks did not run on the checkpoint thread: {threads:?}"
    );
    for name in &threads {
        assert!(
            !name.starts_with("amoru-worker-") && !name.ends_with("-drive"),
