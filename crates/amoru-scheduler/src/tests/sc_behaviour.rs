...
use amoru_kernel::{
    AmoruError, CancelToken, ErrorPolicy, Knob, Knobs, PayloadKind, PayloadSpec, Prober,
...
fn sc_h_zero_kernels() {
    ...
    match rig.scheduler.probe(1, 8) {
        Err(AmoruError::Plan(msg)) => assert!(msg.contains("kernel stages"), "{msg}"),
...
fn sc_h_sink_failure_terminates() {
    ...
    assert!(matches!(diagnostic, AmoruError::Sink(_)), "{diagnostic}");
    assert_eq!(
...
fn sc_h_controller_terminate() {
    ...
    let outcome = std::thread::scope(|scope| {
        ...
        rig.scheduler
            .terminate(AmoruError::Staging("the controller said so".into()));
        match handle.join() {
            Ok(outcome) => outcome,
...
fn sc_i_configuration_is_checked_at_new() {
    ...
    for (config, row) in bad {
        let outcome = RigBuilder::new()
            ...
            .err();
        match outcome {
            Some(AmoruError::Config { name, .. }) => assert_eq!(name, row),
...
fn sc_f1_chain_validation() {
    ...
    impl amoru_kernel::Kernel for TensorKernel {
        fn fingerprint(&self) -> amoru_kernel::Fingerprint {
            amoru_kernel::Fingerprint::compute("tests::TensorKernel", &[])
        }
    ...
    let strings = SourceSchema::Table(Arc::new(amoru_kernel::arrow::datatypes::Schema::new(vec![
        amoru_kernel::arrow::datatypes::Field::new(
            "text",
            amoru_kernel::arrow::datatypes::DataType::Utf8,
            false,
        ),
    ])));
    let outcome = RigBuilder::new()
        ...
        .build();
    match outcome.err() {
        Some(AmoruError::Plan(msg)) => assert!(msg.starts_with("stage 1:"), "{msg}"),
...
fn sc_f1_a_source_that_cannot_be_reread_forces_checkpointing_off() {
    ...
    match rig.scheduler.apply_resume_point(ResumePoint::default()) {
        Err(AmoruError::Resume(msg)) => assert!(msg.contains("source"), "{msg}"),
...
fn sc_e2_run_is_valid_once() {
    ...
    match rig.scheduler.run_resumed(CancelToken::new()) {
        Err(AmoruError::Resume(msg)) => assert!(msg.contains("apply_resume_point"), "{msg}"),
    ...
    match rig.scheduler.run(CancelToken::new()) {
        Err(AmoruError::Plan(msg)) => assert!(msg.contains("only valid once"), "{msg}"),
        other => panic!("a second run is refused, got {other:?}"),
    }
    match rig.scheduler.apply_resume_point(ResumePoint::default()) {
        Err(AmoruError::Resume(msg)) => assert!(msg.contains("before the run starts"), "{msg}"),
...
fn sc_j_stats_before_a_run() {
    ...
    assert_eq!(rig.scheduler.node(), amoru_kernel::LOCAL_NODE);
...
fn sc_d1_ordered_sink_handle() {
    ...
    let scheduler = Scheduler::new(
        ...
        Arc::new(amoru_testkit::FakeAllocator::new()),
        Arc::new(amoru_testkit::FakeTrace::new()),
        Arc::new(amoru_testkit::FakeSampler::new()),
    );
...
fn sc_f13_resume_refusals() {
    ...
    let rig = RigBuilder::new()
        ...
        .go();
    match rig.scheduler.apply_resume_point(ResumePoint::default()) {
        Err(AmoruError::Resume(msg)) => assert!(msg.contains("no state"), "{msg}"),
...
fn sc_f12_three_failed_manifests_end_the_run() {
    ...
    assert!(matches!(diagnostic, AmoruError::Resume(_)), "{diagnostic}");
...
fn sc_f9_probe_with_an_exhausted_plan() {
    let rig = RigBuilder::new()
        ...
        .go();
    match rig.scheduler.probe(1, 8) {
        Err(AmoruError::Plan(msg)) => assert!(msg.contains("exhausted"), "{msg}"),
        other => panic!("expected a Plan error, got {other:?}"),
    }
}
...
fn sc_f5_ordered_sink_completes_in_order() {
    ...
    let scheduler = Scheduler::new(
        ...
        Arc::new(FakePlacement::new()),
        Arc::new(amoru_testkit::FakeAllocator::new()),
        Arc::new(amoru_testkit::FakeTrace::new().capacity(1 << 14)),
        Arc::new(amoru_testkit::FakeSampler::new()),
    );
    let scheduler = match scheduler {
        Ok(scheduler) => scheduler,
...
fn sc_f9_probe_during_a_run() {
    ...
    let probes = rig
        ...
        .records()
        .into_iter()
        .filter(|r| r.outcome == amoru_kernel::Outcome::Probe)
        .count();
...
fn sc_f8_device_out_of_memory_reaches_the_hook_first() {
    ...
    impl amoru_kernel::Kernel for DeviceOom {
        fn fingerprint(&self) -> amoru_kernel::Fingerprint {
            amoru_kernel::Fingerprint::compute("tests::DeviceOom", &[])
        }
        ...
        fn apply(
            &self,
            _state: &mut dyn amoru_kernel::KernelState,
            _input: amoru_kernel::Payload,
        ) -> amoru_kernel::Result<amoru_kernel::Payload> {
            Err(AmoruError::Alloc {
                bytes: 1,
                tier: amoru_kernel::Tier::Device(amoru_kernel::DeviceId(0)),
                budget: 0,
                in_use: 0,
            })
        }
    ...
    rig.scheduler
        .set_record_hook(Arc::new(move |record: &amoru_kernel::TraceRecord| {
            sink_of_records
                .lock()
    ...
    for record in &traced {
        assert_eq!(record.outcome, amoru_kernel::Outcome::Skipped);
    }
