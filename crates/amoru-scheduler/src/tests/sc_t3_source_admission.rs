...
use amoru_kernel::{CancelToken, Knob, Knobs, Morsel, Origin, Payload, TierKind};
use amoru_testkit::{FakeAllocator, FakeKernel, FakeSource};
...
fn morsel(alloc: &FakeAllocator, seq: u64) -> Morsel {
    ...
    let schema = std::sync::Arc::new(amoru_kernel::arrow::datatypes::Schema::new(vec![
...
fn sc_t3_source_admission_q0_full_stops_reads() {
    ...
    for seq in 0..3u64 {
        if let Err(e) =
            amoru_kernel::Placement::push(rig.placement.as_ref(), 0, morsel(&rig.alloc, seq))
        {
            panic!("push: {e}");
        }
    }
    assert!(
        amoru_kernel::Placement::is_full(rig.placement.as_ref(), 0),
