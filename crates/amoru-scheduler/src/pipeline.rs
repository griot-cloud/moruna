...
use amoru_kernel::{
    AmoruError, ErrorPolicy, Kernel, NodeId, PayloadSpec, Result, Sink, Source, SourceSchema,
};
...
pub(crate) fn validate_chain(
    ...
    for (index, kernel) in kernels.iter().enumerate() {
        ...
        spec.check(&schema)
            .map_err(|e| AmoruError::Plan(format!("stage {}: {e}", index as u16 + 1)))?;
        schema = kernel.output_schema(&schema)?;
        schemas.push(schema.clone());
    }
    sink.accepts()
        .check(&schema)
        .map_err(|e| AmoruError::Plan(format!("sink: {e}")))?;
    Ok(schemas)
}
