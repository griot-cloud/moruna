...
use amoru_kernel::{
    AmoruError, BoxFuture, Morsel, Origin, Payload, Result, RowRange, Seq, Split, Tier,
};
...
fn target_stage(shared: &Shared) -> amoru_kernel::StageId {
                ...
                match result {
                    Ok(payload) => {
                        ...
                        if let Err(e) = outcome
                            && !matches!(e, AmoruError::Cancelled)
                        {
...
fn source_diagnostic(e: AmoruError, split: u32, rows: RowRange) -> AmoruError {
    match e {
        AmoruError::Alloc {
            bytes,
            tier,
        ...
        AmoruError::Source { split: at, msg } => AmoruError::Source {
            split: at,
...
fn service_helper(shared: &Shared, request: HelperRequest, parker: &Parker, cx: &mut Context<'_>) {
    ...
    let Some((index, rows, seq, whole)) = take_next_range(shared, Some(request.bytes)) else {
        let _ = reply.send(Err(AmoruError::Plan(
...
pub(crate) fn set_cursor(shared: &Shared, cursor: amoru_kernel::SourceCursor) {
...
pub(crate) fn cursor(shared: &Shared) -> amoru_kernel::SourceCursor {
    let slot = shared.cursor.lock().unwrap_or_else(|e| e.into_inner());
    amoru_kernel::SourceCursor {
