...
use amoru_kernel::{Allocator, AmoruError, Guarantee, TierKind};
...
fn ar_t6_pin_all_or_nothing() {
    ...
    let pinned = arena
        .alloc(4096, amoru_kernel::Tier::PinnedHost)
        .expect("the run's host tier");
    assert_eq!(pinned.tier(), amoru_kernel::Tier::PinnedHost);
    assert_eq!(arena.stats().pinned_in_use, 64 * 1024);
    assert_eq!(arena.stats().host_in_use, 0);
    let e = arena
        .alloc(4096, amoru_kernel::Tier::Host)
        .expect_err("the other host tier does not exist in this run");
    assert!(matches!(e, AmoruError::Alloc { tier, .. } if tier == amoru_kernel::Tier::Host));
    ...
    assert_eq!(host.tier(), amoru_kernel::Tier::Host);
    let e = arena
        .alloc(4096, amoru_kernel::Tier::PinnedHost)
        .expect_err("nothing is pinned in this run");
    assert!(matches!(e, AmoruError::Alloc { tier, .. } if tier == amoru_kernel::Tier::PinnedHost));
    assert!(!arena.arena_stats().pinned);
    drop(host);
    drop(arena);
    ...
    assert!(matches!(e, AmoruError::Config { name, .. } if name == "host_profile"));
