//! Amoru component 2, the memory arena: the only source of payload memory in the process.
//!
...
use amoru_kernel::{
    AllocStats, Allocator, AmoruError, Buffer, DeviceId, Guarantee, Result, Tier, TierKind,
};
...
impl Arena {
    ...
    pub fn new(cfg: ArenaConfig) -> Result<Arc<Arena>> {
        if cfg.register_rdma {
            return Err(AmoruError::Unsupported("rdma"));
        }
    ...
    fn alloc(&self, bytes: usize, tier: Tier) -> Result<Buffer> {
        ...
        match space.alloc(bytes as u64) {
            Ok((ptr, _charged)) => {
                ...
                let handle: Arc<dyn amoru_kernel::ArenaHandle> = Arc::clone(inner) as Arc<_>;
...
mod tests {
    ...
    fn configuration_is_validated() {
        ...
        c.page_bytes = 3;
        assert!(
            matches!(Arena::new(c), Err(AmoruError::Config { name, .. }) if name == "page.bytes")
        );

        let mut c = cfg(1 << 20);
        c.host_tier = TierKind::Device;
        assert!(
            matches!(Arena::new(c), Err(AmoruError::Config { name, .. }) if name == "arena.pin")
        );

        assert!(
            matches!(Arena::new(cfg(1024)), Err(AmoruError::Config { name, .. }) if name == "budget.host")
        );
        ...
        c.host_bytes = (1 << 21) - 1;
        assert!(
            matches!(Arena::new(c), Err(AmoruError::Config { name, .. }) if name == "budget.host")
        );

        let mut c = cfg(1 << 20);
        c.device_bytes = (0..9).map(|i| (DeviceId(i), 1 << 20)).collect();
        assert!(
            matches!(Arena::new(c), Err(AmoruError::Config { name, .. }) if name == "budget.device")
        );
        ...
        #[cfg(not(feature = "cuda"))]
        assert!(
            matches!(r, Err(AmoruError::Config { name, .. }) if name == "budget.device"),
    ...
    fn a_host_arena_serves_its_own_tier_only() {
        ...
        assert!(matches!(e, AmoruError::Alloc { tier, .. } if tier == Tier::PinnedHost));
        let seg = amoru_kernel::SegmentRef {
            segment: 0,
            offset: 0,
            len: 1,
        };
        ...
        assert!(matches!(e, AmoruError::Alloc { tier, .. } if tier == Tier::Disk(seg)));
        let rr = amoru_kernel::RemoteRef {
            addr: 0,
            rkey: 0,
            len: 1,
        };
        let e = arena
            .alloc(1024, Tier::Remote(amoru_kernel::LOCAL_NODE, rr))
            .expect_err("reserved");
        assert!(matches!(e, AmoruError::Unsupported("rdma")));
    ...
    fn a_release_the_arena_did_not_hand_out_is_counted() {
        ...
        let handle: &dyn amoru_kernel::ArenaHandle = arena.inner.as_ref();
        ...
        handle.release(
            heap.as_mut_ptr(),
            8,
            Tier::Remote(amoru_kernel::LOCAL_NODE, rr),
        );
