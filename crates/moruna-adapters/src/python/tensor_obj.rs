...
use moruna_kernel::{DeviceId, ManagedTensor, Tier};
...
#[pyclass(frozen, name = "Tensor", module = "moruna")]
pub struct PyTensor {
...
impl PyTensor {
    ...
    pub fn new(tensor: ManagedTensor, tier: Tier) -> moruna_kernel::Result<PyTensor> {
...
fn dl_device(tier: Tier) -> moruna_kernel::Result<(i32, i32)> {
    match tier {
        Tier::Host => Ok((KDL_CPU, 0)),
        Tier::PinnedHost => Ok((KDL_CUDA_HOST, 0)),
        Tier::Device(DeviceId(id)) => Ok((KDL_CUDA, i32::from(id))),
        Tier::Disk(_) => Err(moruna_kernel::MorunaError::Staging(
            "a tensor on Disk has no bytes to cross into Python".into(),
        )),
        Tier::Remote(_, _) => Err(moruna_kernel::MorunaError::Unsupported("rdma")),
    }
}
...
impl PyTensor {
    ...
    fn __dlpack__(
        ...
        let mut slot = self
            .tensor
            .lock()
            .map_err(|_| PyRuntimeError::new_err("moruna.Tensor lock was poisoned"))?;
        let Some(tensor) = slot.take() else {
            return Err(PyBufferError::new_err(
                "this moruna.Tensor has already been consumed by __dlpack__",
            ));
        };
    ...
    fn __repr__(&self) -> String {
        format!(
            "moruna.Tensor(device_type={}, device_id={})",
