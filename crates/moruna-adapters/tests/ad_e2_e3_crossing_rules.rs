...
fn tensor_values(payload: &Payload) -> Vec<i64> {
    ...
    let bytes = moruna_adapters::python::cross::host_tensor_bytes(tensor).expect("host bytes");
    bytes
        .as_chunks::<8>()
        .0
        .iter()
...
fn the_tensor_object_names_every_tier() {
    ...
    assert!(matches!(
        PyTensor::new(
            tensor,
            Tier::Disk(moruna_kernel::SegmentRef {
                segment: 0,
                offset: 0,
                len: 8,
            })
    ...
    assert!(PyTensor::new(tensor, Tier::Device(moruna_kernel::DeviceId(1))).is_ok());
}
...
fn apply_returning(attribute: &str, module: &str) -> moruna_kernel::Result<Payload> {
    let fake = FakeAllocator::new();
...
fn the_import_rules_name_what_came_back() {
    ...
    let MorunaError::Kernel { msg, .. } = &none else {
    ...
    let MorunaError::Kernel { msg, .. } = &integer else {
    ...
    let MorunaError::Kernel { msg, .. } = &chunks else {
...
fn apply_awkward(attribute: &str, module: &str, rows: &[i64]) -> moruna_kernel::Result<Payload> {
    let counting = CountingAllocator::new(FakeAllocator::new());
    let alloc: Arc<dyn Allocator> = counting.clone();
...
fn a_device_tensor_is_never_copied() {
    let counting = CountingAllocator::new(FakeAllocator::new());
    let alloc: Arc<dyn Allocator> = counting.clone();
    let device = Tier::Device(moruna_kernel::DeviceId(0));
    ...
    let landed = moruna_adapters::python::copy::land(
        moruna_adapters::python::cross::Imported::Tensor(tensor),
        &alloc,
    )
...
fn a_tensor_with_strides_cannot_be_copied() {
    ...
    assert!(
        matches!(error, MorunaError::Convert(_)),
...
fn a_broken_arrow_export_is_an_error() {
    ...
    let MorunaError::Kernel { msg, .. } = &error else {
...
fn a_device_tensor_has_no_host_bytes() {
    let fake = FakeAllocator::new();
    let buffer = fake.buffer(64, Tier::Device(moruna_kernel::DeviceId(0)));
    let tensor = ManagedTensor::from_buffer(buffer, 0, DType::I64, vec![8])
        .expect("a tensor over device memory");
    assert!(matches!(
        moruna_adapters::python::cross::host_tensor_bytes(&tensor),
        Err(MorunaError::Staging(_))
    ));
}
