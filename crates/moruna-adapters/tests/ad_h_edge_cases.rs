...
fn apply_before_bind_allocator_is_a_config_error() {
    ...
    let MorunaError::Config { name, .. } = &error else {
...
fn a_stateless_kernel_must_be_callable() {
    ...
    assert!(matches!(error, MorunaError::Plan(_)));
}
...
fn a_batch_that_is_not_resident_cannot_cross() {
    ...
    Python::attach(|py| {
        ...
        assert!(matches!(export(py, staged), Err(MorunaError::Staging(_))));
        ...
        assert!(matches!(
            export(py, device),
            Err(MorunaError::Unsupported("arrow-c-device"))
        ));
    });
}
...
fn the_ctx_object_carries_the_instance_and_the_device() {
    ...
    let py_state = seen
        .downcast_mut::<moruna_adapters::python::PyState>()
