...
use amoru_kernel::{Allocator, AmoruError, InitCtx, Kernel, KernelKind, Payload, ResumePolicy};
...
fn ad_t12_instances_are_independent_and_apply_gets_the_matching_state() {
    ...
    let mut states: Vec<Box<dyn amoru_kernel::KernelState>> = (0..INSTANCES)
        .map(|instance| {
            kernel
...
fn ad_t12_a_class_without_setup_or_call_is_refused_when_the_kernel_is_built() {
    for (source, attribute, module, missing) in [
        ...
        let AmoruError::Plan(message) = &error else {
