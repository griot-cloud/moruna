...
use amoru_kernel::arrow::array::{Array, ArrayData, ArrayRef, make_array};
use amoru_kernel::arrow::buffer::{BooleanBuffer, Buffer as ArrowBuffer, NullBuffer};
use amoru_kernel::arrow::record_batch::RecordBatch;
use amoru_kernel::{Allocator, AmoruError, ManagedTensor, Payload, Result, Tier};
...
fn land_table(batch: RecordBatch, alloc: &dyn Allocator) -> Result<Landed> {
    ...
    .map_err(|e| AmoruError::Plan(format!("boundary copy: {e}")))?;
    Ok(Landed {
...
fn copy_array_data(
    ...
    builder
        .build()
        .map_err(|e| AmoruError::Plan(format!("boundary copy: rebuilding an array: {e}")))
}
...
fn land_tensor(tensor: ManagedTensor, alloc: &dyn Allocator) -> Result<Landed> {
    let resident_on_host = match tensor.tier() {
        ...
        Tier::Remote(_, _) => return Err(AmoruError::Unsupported("rdma")),
    };
    if !resident_on_host {
        return Ok(Landed {
