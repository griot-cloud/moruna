//! `PyKernel`: a Python callable or class as an Amoru [`Kernel`] (d.1, f.1, f.2, f.4).
...
use amoru_kernel::{
    Allocator, AmoruError, Fingerprint, GilState, InitCtx, Kernel, KernelHints, KernelKind,
...
/// The stage an `AmoruError::Kernel` from this crate carries.
///
...
/// The morsel an `AmoruError::Kernel` from this crate carries; see [`UNKNOWN_STAGE`].
pub const UNKNOWN_SEQ: Seq = Seq::MAX;

const GIL_FREE_THREADED: u8 = 0;
const GIL_SERIALISED: u8 = 1;

/// An `AmoruError::Kernel` from the adapter, with the sentinel stage and morsel.
pub(crate) fn kernel_error(msg: String) -> AmoruError {
    AmoruError::Kernel {
        stage: UNKNOWN_STAGE,
        seq: UNKNOWN_SEQ,
        msg,
...
/// A Python callable or class as an Amoru kernel.
pub struct PyKernel {
    callable: Py<PyAny>,
    stateful: bool,
