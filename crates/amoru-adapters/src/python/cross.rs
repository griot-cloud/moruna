...
use amoru_kernel::{AmoruError, Dlpack, ManagedTensor, Payload, Result, Tier};
use pyo3::prelude::*;
...
pub fn export<'py>(py: Python<'py>, input: Payload) -> Result<Bound<'py, PyAny>> {
    match input {
        Payload::Table(batch, tier) => match tier {
            ...
            Tier::Device(_) => Err(AmoruError::Unsupported("arrow-c-device")),
            Tier::Disk(_) => Err(AmoruError::Staging(
