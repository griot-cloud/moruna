...
use amoru_kernel::{AmoruError, DeviceId, KernelState, Result, ResumePolicy};
use pyo3::prelude::*;
...
/// `AmoruError::Resume` for a kernel that declares `Checkpoint` and whose `restore` is missing at
...
pub(crate) fn missing_restore() -> AmoruError {
    AmoruError::Resume("the Python kernel declares Checkpoint but has no restore".into())
}
