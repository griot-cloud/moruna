...
use moruna_sinks::{ArrowIpcSink, ParquetSink, TensorSink};
use moruna_sources::{ParquetSource, TensorSource};
...
impl RunningGuard {
    fn acquire() -> Result<RunningGuard, MorunaError> {
...
fn resolve_resume(t: &Translated) -> Result<Option<PathBuf>, MorunaError> {
...
fn run_id(hex: &str) -> Result<moruna_kernel::RunId, MorunaError> {
...
fn source_factory(
    source: SourceSpec,
    iterator: Option<Py<PyAny>>,
) -> Result<moruna_runtime::SourceSpec, MorunaError> {
    Ok(match source {
        ...
        SourceSpec::Iterator { schema } => {
            ...
            moruna_runtime::SourceSpec::Build(Box::new(move |ctx| {
...
fn sink_factory(sink: SinkSpec) -> moruna_runtime::SinkSpec {
    match sink {
        SinkSpec::Parquet(cfg) => moruna_runtime::SinkSpec::Build(Box::new(move |ctx| {
        ...
        SinkSpec::Tensor(cfg) => moruna_runtime::SinkSpec::Build(Box::new(move |ctx| {
        ...
        SinkSpec::ArrowIpc(cfg) => moruna_runtime::SinkSpec::Build(Box::new(move |ctx| {
...
mod tests {
    ...
    fn resume_needs_a_place_to_look() {
        ...
        .expect("translate");
        assert!(matches!(resolve_resume(&t), Err(MorunaError::Resume(_))));
