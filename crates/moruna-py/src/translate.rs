//! `moruna.run` argument translation and its range clamping (f.3, PY-I10).
//!
...
fn config(name: &'static str, msg: String) -> MorunaError {
...
mod tests {
    use super::*;
    use moruna_sinks::{ParquetSinkConfig, TensorSinkConfig};
    ...
    /// arguments of `moruna.run`; each arrives at the bound and exactly one note names the clamp.
    #[test]
    fn py_t13_configuration_clamping() {
        ...
        assert!(matches!(
            bad_sizer,
            Err(MorunaError::Config { name: "sizer", .. })
        ));
    }
    ...
    fn py_t14_sink_equals_source() {
        ...
        assert!(matches!(err, MorunaError::Plan(_)), "{err}");
        assert!(
        ...
        let tensor_sink = SinkSpec::Tensor(TensorSinkConfig {
            ...
            format: moruna_sinks::TensorFormat::Amb1,
    ...
    fn trace_path_validity() {
        ...
        let file = dir.join(format!("moruna-py-trace-{}.arrow", std::process::id()));
        assert_eq!(trace_path(&file.to_string_lossy()).expect("a file"), file);
        let missing = dir.join("no-such-moruna-dir").join("t.arrow");
        assert!(matches!(
    ...
    fn sizes_and_paths_translate() {
        let t = translate(
            RawArgs {
                staging_dir: Some("/tmp/moruna-staging".into()),
                profiles_dir: Some("/tmp/moruna-profiles".into()),
        ...
        .expect("translate");
        assert_eq!(t.staging_dir, Some(PathBuf::from("/tmp/moruna-staging")));
        assert_eq!(t.profiles_dir, Some(PathBuf::from("/tmp/moruna-profiles")));
