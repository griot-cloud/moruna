...
pub enum AmoruError {
    ...
    Io {
        /// The operation (`read_file`, `amb1`, `ipc`, ...).
        op: &'static str,
...
mod tests {
    ...
    fn messages_carry_the_morsel() {
        ...
        let s = AmoruError::Source {
            split: 3,
            msg: "gone".into(),
        };
        assert_eq!(s.to_string(), "source split 3: gone");
        let c = AmoruError::Config {
            name: "budget.host",
            msg: "low".into(),
        };
        assert_eq!(c.to_string(), "config budget.host: low");
        let io = AmoruError::Io {
            op: "amb1",
            target: "f".into(),
            msg: "short".into(),
        };
        assert_eq!(io.to_string(), "io amb1 f: short");
        for (e, text) in [
            (AmoruError::Plan("p".into()), "plan: p"),
            (AmoruError::Sink("s".into()), "sink: s"),
            (AmoruError::Staging("g".into()), "staging: g"),
            (AmoruError::Cancelled, "cancelled"),
            (AmoruError::Resume("r".into()), "resume: r"),
            (AmoruError::Unsupported("rdma"), "unsupported: rdma"),
            (
