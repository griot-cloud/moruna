//! The Amoru aligned binary format `AMB1` (contracts e.4): the header reader and writer over
...
impl Header {
    ...
    /// `buf.len() >= data_offset + payload_len`. Any failure is `Io { op: "amb1" }`; an unknown
    ...
    pub fn read(buf: &[u8]) -> crate::Result<Header> {
        ...
        if buf[0..4] != MAGIC {
            return Err(bad("magic is not AMB1"));
        }
    ...
    /// The payload bytes of a validated `AMB1` buffer.
...
/// The bytes a whole `AMB1` record occupies for a tensor of this rank and size: the header,
