...
impl DType {
    ...
    /// The `AMB1` dtype code (e.4): I8=0, I16=1, I32=2, I64=3, U8=4, U16=5, U32=6, U64=7,
    ...
    /// The dtype for an `AMB1` code; `None` for an unknown code.
            ...
            (PayloadKind::Tensor, SourceSchema::Table(schema)) => {
                ...
                for field in schema.fields() {
                    let dtype = column_dtype(field.data_type()).ok_or_else(|| {
                        AmoruError::Plan(format!("column {} is not numeric", field.name()))
                    })?;
...
impl Payload {
    ...
    pub fn as_tensor(&self, column: Option<&str>) -> crate::Result<ManagedTensor> {
        ...
        if !tier.is_resident() {
            return Err(AmoruError::Staging("payload not resident".into()));
        }
        match column {
            Some(name) => {
                let index = batch
                    .schema()
                    .index_of(name)
                    .map_err(|_| AmoruError::Plan(format!("as_tensor: no column named {name}")))?;
    ...
    pub fn as_column(&self, name: &str) -> crate::Result<ArrayRef> {
        ...
        let values = match std::ptr::NonNull::new(ptr) {
            ...
            None => return Err(AmoruError::Staging("tensor data pointer is null".into())),
        };
        ...
        let primitive = ArrayData::builder(arrow_type.clone())
            ...
            .add_buffer(values)
            .build()
            .map_err(|e| AmoruError::Plan(format!("as_column: {e}")))?;
        if shape.len() == 1 {
            return Ok(make_array(primitive));
        }
        let width = i32::try_from(shape[1])
            .map_err(|_| AmoruError::Plan(format!("as_column: width {} exceeds i32", shape[1])))?;
    ...
    pub fn with_column(self, name: &str, array: ArrayRef) -> crate::Result<Payload> {
        ...
        let batch = RecordBatch::try_new(new_schema, columns)
            .map_err(|e| AmoruError::Plan(format!("with_column: {e}")))?;
        Payload::table(batch)
    }
}
