...
//! `AMORU_REFERENCE_HOST=1` names this host as the reference host; the structural half runs
...
use amoru_kernel::{MorselFeatures, Payload, Tier};
...
fn ct_t11_features_cost() {
    ...
    let tensor = amoru_kernel::ManagedTensor::from_buffer(
        alloc.buffer(4 * 4, Tier::Host),
        0,
        amoru_kernel::DType::F32,
        vec![2, 2],
    )
    ...
    assert_eq!(tensor.dtype, Some(amoru_kernel::DType::F32));
    ...
    if std::env::var("AMORU_REFERENCE_HOST").as_deref() == Ok("1") {
