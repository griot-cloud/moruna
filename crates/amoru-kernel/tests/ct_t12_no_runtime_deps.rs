//! CT-T12 no_runtime_deps: `cargo tree -p amoru-kernel` contains none of tokio, cudarc, pyo3,
//! parquet or object_store. Proves the boundary in section a and S7.

use std::process::Command;

/// The crates a kernel author's dependency on `amoru-kernel` must never pull in (section a).
...
fn ct_t12_no_runtime_deps() {
    ...
    for forbidden in FORBIDDEN {
        assert!(
            ...
            "amoru-kernel must not depend on {forbidden} (01 section a, CT-T12):\n{tree}"
        );
    }
