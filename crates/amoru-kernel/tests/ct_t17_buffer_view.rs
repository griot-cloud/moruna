...
use amoru_kernel::{AmoruError, BufferView, DType, ManagedTensor, Tier};
...
fn ct_t17_buffer_view() {
    ...
    let device = common::tagged_buffer(64, Tier::Device(amoru_kernel::DeviceId(1)));
    let tensor = Arc::new(
    ...
    assert_eq!(view.tier(), Tier::Device(amoru_kernel::DeviceId(1)));
    ...
    assert!(
        matches!(err, AmoruError::Convert(_)),
