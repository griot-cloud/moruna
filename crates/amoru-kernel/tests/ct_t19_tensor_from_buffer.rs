//! CT-T19 tensor_from_buffer: `ManagedTensor::from_buffer` over an `AMB1` body gives
...
use amoru_kernel::amb1::{Header, record_len};
use amoru_kernel::{AmoruError, DType, ManagedTensor, Payload, Tier};
...
fn ct_t19_tensor_from_buffer() {
    ...
    // The staging path of placement f.6: an AMB1 record in one buffer, the tensor built over
    ...
    let elsewhere = common::tagged_buffer(64, Tier::Remote(amoru_kernel::LOCAL_NODE, remote));
    ...
    let device = common::tagged_buffer(32, Tier::Device(amoru_kernel::DeviceId(2)));
    let tensor = ManagedTensor::from_buffer(device, 0, DType::I32, vec![8])
        .expect("a tensor over a device buffer");
    assert_eq!(tensor.tier(), Tier::Device(amoru_kernel::DeviceId(2)));
