...
//!
//! Every error reaches Python as one exception type with a structured payload: `AmoruError` with
//! `.kind` (the `AmoruError` variant name), `.message` and `.diagnostic`, plus `.run_id`,
//! `.manifest` and `.report` when the run produced them. The subclasses exist for `except`
//! ergonomics and are exactly e.2's table.

use amoru_kernel::{AmoruError as RsError, MorselFeatures};
...
create_exception!(_core, KernelError, AmoruError, "A kernel raised.");
create_exception!(
    _core,
    BudgetError,
    AmoruError,
...
/// The `AmoruError` variant's name, which becomes `.kind` (PY-I2).
...
/// Map an `AmoruError` to its Python exception, with the structured payload PY-I2 requires.
...
pub fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    let py = m.py();
    m.add("AmoruError", py.get_type::<AmoruError>())?;
...
mod tests {
    use super::*;
    use amoru_kernel::{ConvertError, Tier};
    ...
    /// `AmoruError` variant arrives as the class e.2 names, with `.kind` and the diagnostic dict
    /// populated.
    #[test]
    fn py_t2_exception_mapping() {
        ...
        Python::attach(|py| {
            ...
            for (err, class_name, kind) in cases {
                ...
                assert!(py_err.is_instance_of::<AmoruError>(py), "{kind}");
