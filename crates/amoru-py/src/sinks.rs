...
#[pyclass(frozen, module = "amoru._core", name = "TensorSink")]
...
impl PyTensorSink {
    #[new]
    #[pyo3(signature = (path, *, format = "amb1", one_file_per_morsel = false, name = "tensor"))]
    fn new(
        path: String,
        format: &str,
        ...
        let format = match format.to_ascii_lowercase().as_str() {
            "amb1" => TensorFormat::Amb1,
            "safetensors" => TensorFormat::SafeTensors,
            other => {
                return Err(pyo3::exceptions::PyValueError::new_err(format!(
                    "unknown tensor format `{other}` (use \"amb1\" or \"safetensors\")"
                )));
            }
        };
        Ok(PyTensorSink {
...
#[pyclass(frozen, module = "amoru._core", name = "ArrowIpcSink")]
...
pub fn spec_of(handle: &Bound<'_, PyAny>) -> PyResult<(SinkSpec, Vec<String>)> {
    ...
    Err(pyo3::exceptions::PyTypeError::new_err(format!(
        "sink must be an amoru.ParquetSink, amoru.TensorSink or amoru.ArrowIpcSink, not {}",
        type_name(handle)
    )))
}
