...
use amoru_sources::{ParquetSourceConfig, RowFilter, ScalarValue, TensorSourceConfig};
use pyo3::prelude::*;
...
#[pyclass(frozen, module = "amoru._core", name = "ParquetSource")]
...
#[pyclass(frozen, module = "amoru._core", name = "TensorSource")]
...
#[pyclass(frozen, module = "amoru._core", name = "IteratorSource")]
...
pub fn spec_of(handle: &Bound<'_, PyAny>) -> PyResult<SourceSpec> {
    ...
    Err(pyo3::exceptions::PyTypeError::new_err(format!(
        "source must be an amoru.ParquetSource, amoru.TensorSource or amoru.IteratorSource, not {}",
        type_name(handle)
    )))
}
