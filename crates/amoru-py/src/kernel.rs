//! `@amoru.kernel`: the decorator's product (b, d.2, l).
//!
...
use amoru_adapters::{PyKernel, PyKernelSpec};
use amoru_kernel::{PayloadKind, PayloadSpec, ResumePolicy, TierPref};
use pyo3::prelude::*;
...
/// What `@amoru.kernel` produces: a frozen handle over one `PyKernel` (b).
#[pyclass(frozen, module = "amoru._core", name = "KernelSpec")]
...
#[pymethods]
impl PyKernelHandle {
    /// Call the decorated object, so `@amoru.kernel` does not take a function away from its module.
...
/// Build the kernel object `@amoru.kernel` returns (d.2). The Python half applies the decorator;
...
/// The kernels of one run, in stage order, from whatever `amoru.run` was given: one kernel, a
...
pub fn kernels_of(py: Python<'_>, value: &Bound<'_, PyAny>) -> PyResult<Vec<Arc<PyKernel>>> {
    ...
    for item in items {
        ...
        return Err(pyo3::exceptions::PyTypeError::new_err(format!(
            "a kernel must be produced by @amoru.kernel or be callable, and {} is neither",
            type_name(&item)
        )));
    }
    Ok(out)
...
/// The decorator's Rust half, exposed as `_core.build_kernel`; `amoru.kernel` applies it.
#[pyfunction]
