...
use moruna_kernel::{Knobs, Morsel, Origin, Payload};
...
fn morsel(alloc: &FakeAllocator, seq: u64, stage: u16, bytes: usize) -> Morsel {
    ...
    let data = moruna_kernel::arrow::array::ArrayData::builder(
        moruna_kernel::arrow::datatypes::DataType::UInt8,
    )
    .len(bytes)
    .add_buffer(buffer)
    .build();
...
fn sc_t2_admission_rule() {
    ...
    let place = |stage: u16, seq: u64, bytes: usize| {
        if let Err(e) = moruna_kernel::Placement::push(
    ...
    rig.scheduler.set(moruna_kernel::Knob::HighWater {
        stage: 3,
