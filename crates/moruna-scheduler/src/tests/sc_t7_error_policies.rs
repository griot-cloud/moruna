...
fn seqs_with(records: &[moruna_kernel::TraceRecord], outcome: Outcome) -> Vec<Seq> {
    records
        .iter()
...
fn run(
    policy: ErrorPolicy,
    fail_on: &[usize],
) -> (Vec<moruna_kernel::TraceRecord>, Vec<Seq>, crate::RunOutcome) {
