...
//!
//! The instrumentation is the thread name: the scheduler names its threads `moruna-worker-N`,
//! `moruna-source-drive` and `moruna-sink-drive`, and the test records the name of the thread
...
fn sc_t1_workers_only_apply() {
    ...
    let sink = NamingSink::new(moruna_testkit::FakeSink::new());
    ...
    assert_eq!(
        reads,
        vec!["moruna-source-drive".to_string()],
        "reads: {reads:?}"
    );
    assert_eq!(
        writes,
        vec!["moruna-sink-drive".to_string()],
        "writes: {writes:?}"
    );
    assert!(!applies.is_empty(), "the kernel ran");
    for name in &applies {
        assert!(
            name.starts_with("moruna-worker-"),
