...
use amoru_kernel::{AmoruError, ObjectMeta, Result};
use bytes::Bytes;
...
/// (RE-I1, f.4).
struct ViewBytes(amoru_kernel::BufferView);
...
fn payload_of(view: amoru_kernel::BufferView) -> PutPayload {
...
pub(crate) async fn write(
    ...
    let mut failure: Option<AmoruError> = None;
...
pub(crate) fn config(msg: &str) -> AmoruError {
    AmoruError::Config {
        name: "object_store",
        msg: msg.to_string(),
    }
...
pub(crate) fn io(op: &'static str, url: &str, e: &object_store::Error) -> AmoruError {
    AmoruError::Io {
        op,
        target: url.to_string(),
        msg: e.to_string(),
...
mod tests {
    ...
    fn an_unknown_scheme_or_a_missing_backend_is_a_config_error() {
        ...
        assert!(matches!(l.parse("s3:///k"), Err(AmoruError::Config { .. })));
