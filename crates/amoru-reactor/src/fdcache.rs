...
pub(crate) fn io_error(op: &'static str, target: &Path, e: &io::Error) -> AmoruError {
    AmoruError::Io {
        op,
...
pub(crate) mod tests {
    ...
    pub(crate) fn scratch_dir(tag: &str) -> PathBuf {
        ...
        let dir =
            std::env::temp_dir().join(format!("amoru-reactor-{}-{tag}-{n}", std::process::id()));
    ...
    fn descriptors_are_cached_and_forgettable() {
        ...
        assert!(matches!(err, AmoruError::Io { op: "open", .. }));
        std::fs::remove_dir_all(&dir).ok();
    }
}
