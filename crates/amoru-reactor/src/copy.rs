...
use amoru_kernel::{AmoruError, Buffer, CopyDst, CopySrc, DeviceId, Result, SegmentRef, Tier};
...
mod tests {
    ...
    fn remote_buffer(len: usize, tier: Tier) -> Buffer {
        struct NoArena;
        impl amoru_kernel::ArenaHandle for NoArena {
    ...
    fn a_remote_endpoint_is_unsupported_in_every_v1_build() {
        ...
        assert!(matches!(err, AmoruError::Unsupported("rdma")));
        let err = plan(
            &view(&alloc, alloc.host_tier(), 64),
