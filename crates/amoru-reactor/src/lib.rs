//! Amoru component 6, the IO reactor: where every byte enters and leaves the process and where
...
use amoru_kernel::{
    Allocator, AmoruError, Buffer, BufferView, Completion, CopyDst, CopySrc, DeviceId, HostProfile,
...
fn select_file_engine(
    ...
    match started {
        ...
        Ok(None) | Err(_) if paths.uring.guaranteed => Err(AmoruError::Config {
            name: "host_profile",
...
impl amoru_kernel::Reactor for Reactor {
...
impl Drop for Reactor {
    fn drop(&mut self) {
        amoru_kernel::Reactor::shutdown(self);
    }
}
