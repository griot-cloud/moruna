...
use amoru_kernel::{
    AmoruError, Buffer, CopyDst, CopySrc, DeviceId, Guarantee, HostProfile, ObjectMeta,
...
fn re_t7_shutdown_completes() {
    ...
    assert!(matches!(after, Err(AmoruError::Cancelled)));
    assert!(matches!(
...
fn object_metadata_answers_head_and_list() {
    ...
    assert!(matches!(bad, Err(AmoruError::Config { .. })));
    reactor.shutdown();
}
...
fn a_file_operation_refuses_a_destination_it_cannot_address() {
    ...
    assert!(matches!(err, AmoruError::Staging(_)));
...
fn queued_operations_are_cancelled_by_shutdown() {
    ...
    for _ in 0..40 {
        ...
        completions.push(Box::new(move || {
            matches!(c.wait(), Err(AmoruError::Cancelled))
        }));
...
fn an_unpinned_arena_without_a_bounce_buffer_refuses_a_device_copy() {
    ...
    assert!(matches!(
        err,
        AmoruError::Staging(_) | AmoruError::Io { op: "copy", .. }
    ));
    reactor.shutdown();
}
...
fn an_object_read_refuses_what_it_cannot_fill() {
    ...
    assert!(matches!(err, AmoruError::Io { .. }));
    let err = reactor
        .read_object(
            "s3://bucket/o",
            0,
