...
fn apply_huge_pages(m: &Mapping, huge_pages: Guarantee) -> Result<bool> {
    match huge_pages {
        Guarantee::Unknown => Err(MorunaError::Config {
            name: "host_profile",
        ...
        Guarantee::Present | Guarantee::Probed(true) => {
            ...
            match (err, huge_pages) {
                (None, _) => Ok(true),
                (Some(msg), Guarantee::Present) => Err(MorunaError::Config {
                    name: "host_profile",
...
fn apply_memlock(m: &Mapping, memlock: Guarantee) -> Result<bool> {
    match memlock {
        ...
        Guarantee::Absent | Guarantee::Probed(false) => Err(MorunaError::Config {
            name: "arena.pin",
...
/// then a `Config` error.
#[cfg(feature = "cuda")]
pub(crate) fn new_device(id: moruna_kernel::DeviceId, bytes: u64) -> Result<(Mapping, u64)> {
...
mod tests {
    ...
    fn an_unknown_guarantee_is_a_config_error() {
        ...
        assert!(matches!(r, Err(MorunaError::Config { name, .. }) if name == "host_profile"));
        let r = new_host(
            1 << 20,
            1 << 16,
            true,
    ...
    fn pinning_without_memlock_is_a_config_error() {
        ...
        assert!(matches!(r, Err(MorunaError::Config { name, .. }) if name == "arena.pin"));
    }
    ...
    fn a_declared_huge_page_guarantee_that_fails_is_a_config_error() {
        ...
        match r {
            Err(MorunaError::Config { name, .. }) => assert_eq!(name, "host_profile"),
