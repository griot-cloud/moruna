//! Moruna component 8, the sinks (`Sink`, `SinkHandle`, `ReorderBuffer`).
//!
...
impl Phase {
    ...
    pub(crate) fn require_open(&self) -> Result<()> {
        match self {
            Phase::Open => Ok(()),
            Phase::Created => Err(MorunaError::Sink("the sink is not open".into())),
            Phase::Failed(msg) => Err(MorunaError::Sink(format!("the sink failed: {msg}"))),
            Phase::Finished => Err(MorunaError::Sink("the sink is finished".into())),
        }
    }
    ...
    pub(crate) fn require_created(&self) -> Result<()> {
        match self {
            Phase::Created => Ok(()),
            Phase::Open => Err(MorunaError::Sink("the sink is already open".into())),
            Phase::Failed(msg) => Err(MorunaError::Sink(format!("the sink failed: {msg}"))),
            Phase::Finished => Err(MorunaError::Sink("the sink is finished".into())),
        }
    }
...
pub(crate) fn require_host(payload: &Payload) -> Result<()> {
    match payload.tier() {
        ...
        Tier::Device(_) => Err(MorunaError::Sink("device payload".into())),
        Tier::Disk(_) => Err(MorunaError::Sink("disk payload".into())),
