...
//! `moruna_kernel::ipc::encode_framing` and by nothing else, so a segment body and an IPC file
...
impl ArrowIpcSink {
    ...
    fn close_file(&self, inner: &mut Inner, file_id: u64) -> Result<Pending> {
        let Some(file) = inner.files.remove(&file_id) else {
            return Err(MorunaError::Sink("the sink has no such file".into()));
        };
...
fn own_buffers(batch: &RecordBatch) -> BTreeSet<usize> {
    fn walk(data: &moruna_kernel::arrow::array::ArrayData, out: &mut BTreeSet<usize>) {
...
/// to this file's offsets in place, the same way `moruna_kernel::ipc` rewrites buffer entries.
fn build_footer(
    schema: &SchemaRef,
    blocks: &[(i64, i32, i64)],
    run_id: &RunId,
    ...
    let bad = |e: String| MorunaError::Sink(format!("arrow ipc footer: {e}"));
    let mut template: Vec<u8> = Vec::new();
    let mut writer = FileWriter::try_new(&mut template, schema).map_err(|e| bad(e.to_string()))?;
    writer.write_metadata("moruna.run_id", hex(run_id));
    if let Some((seq_min, seq_max)) = seq_range {
        writer.write_metadata("moruna.seq_min", seq_min.to_string());
        writer.write_metadata("moruna.seq_max", seq_max.to_string());
    }
...
fn rewrite_blocks(footer: &mut [u8], blocks: &[(i64, i32, i64)]) -> Result<()> {
    let bad = |e: &str| MorunaError::Sink(format!("arrow ipc footer: {e}"));
    if blocks.is_empty() {
        return Ok(());
    }
    let start = {
        let parsed = moruna_kernel::arrow::ipc::root_as_footer(footer)
    ...
    for (i, (offset, meta, body)) in blocks.iter().enumerate() {
        let block = moruna_kernel::arrow::ipc::Block::new(*offset, *meta, *body);
