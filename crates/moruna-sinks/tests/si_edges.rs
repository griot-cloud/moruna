...
fn a_payload_that_is_not_resident_is_refused() {
    ...
    let elsewhere = unsafe {
        Payload::table_in(
            ...
            Tier::Remote(moruna_kernel::LOCAL_NODE, remote),
        )
    };
    let outcome = block_on(sink.write(1, elsewhere));
    assert!(
        matches!(outcome, Err(MorunaError::Unsupported("rdma"))),
...
fn a_morsel_too_large_to_encode_is_named() {
    ...
    let Err(MorunaError::Sink(msg)) = refused else {
...
fn a_tensor_sink_refuses_drift_and_reports_failure() {
    ...
    assert!(matches!(
        sink.finish(),
        Err(MorunaError::Sink(_)) // finish ran twice
    ));
}
...
fn a_per_morsel_tensor_sink_resumes() {
    ...
    drop(first);
    materialise(&reactor, scratch.path());
    std::fs::write(scratch.path().join("w-000000000009.mrb1.tmp"), b"partial").expect("write");
    ...
    assert!(names.iter().all(|n| n.ends_with(".mrb1")));
...
fn finish_before_open_and_after_failure() {
    ...
    .expect("tensor sink");
    assert!(matches!(sink.finish(), Err(MorunaError::Sink(_))));
    ...
    .expect("tensor sink");
    sink.open(&tensor_source_schema(4)).expect("open");
    std::fs::write(scratch.path().join("tensor.mrb1.tmp"), b"partial").expect("write");
