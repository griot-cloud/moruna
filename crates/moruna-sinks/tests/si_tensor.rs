...
fn si_t6_device_rejected() {
    use moruna_kernel::{DeviceId, Payload};
...
/// SI-T10. The byte-level half, which needs no other component: a per-morsel MRB1 file parses
...
fn si_t10_amb1_and_safetensors() {
    // MRB1, one file per morsel: the sequence number is the file name (e.4).
    let scratch = Scratch::new("t10-mrb1");
    let (mut sink, reactor, alloc) = new_sink(&scratch, TensorFormat::Amb1, true);
    sink.open(&tensor_source_schema(4)).expect("open");
    for seq in 0..3 {
    ...
    assert_eq!(
        summary.files,
        vec![
            "weights-000000000000.mrb1".to_string(),
            "weights-000000000001.mrb1".to_string(),
            "weights-000000000002.mrb1".to_string(),
        ]
    );
    ...
    for (seq, name) in summary.files.iter().enumerate() {
        ...
        let header = mrb1::Header::read(&bytes).expect("the contracts reader parses the header");
    ...
    // MRB1, one file for the run: the header is written at `finish`, with the total shape.
    let scratch = Scratch::new("t10-mrb1-run");
    ...
    let summary = sink.finish().expect("finish");
    assert_eq!(summary.files, vec!["weights.mrb1".to_string()]);
    let bytes = written(&reactor, scratch.path(), "weights.mrb1");
    let header = mrb1::Header::read(&bytes).expect("the contracts reader parses the header");
...
fn si_t10_amb1_and_safetensors_roundtrip() {
    unimplemented!("needs moruna-sources TensorSource, which lands in the same wave");
}
...
fn tensor_schema_drift_is_refused() {
    ...
    assert!(matches!(outcome, Err(MorunaError::Sink(_))), "{outcome:?}");
}
...
fn an_empty_tensor_run_writes_a_header() {
    ...
    assert_eq!(summary.files, vec!["weights.mrb1".to_string()]);
    let bytes = written(&reactor, scratch.path(), "weights.mrb1");
    let header = mrb1::Header::read(&bytes).expect("header");
