...
fn si_t2_finish_once() {
    ...
    assert!(matches!(after, Err(MorunaError::Sink(_))), "{after:?}");
...
fn si_t3_no_partial_final() {
    ...
    assert!(matches!(failed, MorunaError::Io { .. }), "{failed:?}");
...
fn si_t7_summary_exact() {
    ...
    assert!(kv.iter().any(|k| k.key == "moruna.run_id"));
    assert!(kv.iter().any(|k| k.key == "moruna.seq_min"));
    assert!(kv.iter().any(|k| k.key == "moruna.seq_max"));
}
...
fn si_t8_parquet_roundtrip() {
    unimplemented!("needs moruna-sources ParquetSource, which lands in the same wave");
}
...
fn schema_drift_names_the_field() {
    use moruna_kernel::Payload;
    use moruna_kernel::arrow::array::{ArrayData, make_array};
