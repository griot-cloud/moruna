...
use amoru_kernel::{Device, Guarantee, LimitSource, Limits, TierKind};
...
pub(crate) struct LimitsInput<'a> {
    /// A budget from the constructor or `AMORU_BUDGET` (DS-I1).
    pub(crate) explicit_budget: Option<u64>,
    /// A CPU quota from the constructor or `AMORU_CPU` (DS-I1).
