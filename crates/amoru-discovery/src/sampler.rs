//! The `amoru_kernel::Sampler` implementation (d.1, f.2, DS-I3, DS-I4).
//!
//! One instance per run, shared as `Arc<dyn amoru_kernel::Sampler>` by the controller thread and
...
use amoru_kernel::{Device, Result, Sample, Sampler as SamplerTrait};
...
fn open(path: &Path) -> Result<File> {
    File::open(path).map_err(|err| amoru_kernel::AmoruError::Io {
        op: "open",
...
mod tests {
    ...
    use amoru_kernel::{HostProfile, LimitSource, Limits, TierKind};
    use std::sync::Arc;
    ...
    fn new_fails_on_an_unreadable_cgroup() {
        ...
        assert!(matches!(
            err,
            amoru_kernel::AmoruError::Io { op: "open", .. }
        ));
    }
