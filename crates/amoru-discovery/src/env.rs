...
use amoru_kernel::{AmoruError, Guarantee, HostProfile, Result};
...
pub(crate) fn config(name: &'static str, msg: impl Into<String>) -> AmoruError {
    AmoruError::Config {
        name,
        msg: msg.into(),
    }
...
/// Parse `AMORU_HOST_PROFILE` (e.1). A comma separated list of `key=value` pairs; an unknown key,
...
/// Parse a CPU quota (`AMORU_CPU`): a positive, finite number of cores.
...
mod tests {
    ...
    /// `AMORU_CPU` accepts a positive number of cores and nothing else.
    #[test]
    fn cpu_strings() {
    ...
    fn map_env_reads_back() {
        let env = MapEnv::with(&[("AMORU_BUDGET", "1GiB")]);
        assert_eq!(env.get("AMORU_BUDGET").as_deref(), Some("1GiB"));
        assert_eq!(env.get("AMORU_CPU"), None);
        assert_eq!(MapEnv::default().get("AMORU_BUDGET"), None);
        // The process environment is readable and returns None for a name nothing sets.
        assert_eq!(ProcessEnv.get("AMORU_DEFINITELY_NOT_SET_9d1f"), None);
    }
}
