//! Amoru component 3, resource discovery and the host profile (`Limits`, `HostProfile`, `Sampler`).
//!
...
// Every fallible function here returns the contract's `Result<T>`, whose error is `AmoruError`.
...
use amoru_kernel::{Guarantee, HostProfile, Limits, Result, TierKind};
...
fn discover_with(env: &dyn EnvSource, roots: &Roots, input: &DiscoveryInput) -> Result<Discovered> {
    ...
    let budget_from_env = match env.get("AMORU_BUDGET") {
    ...
    if env.get("DATABRICKS_RUNTIME_VERSION").is_some() {
        if explicit_budget.is_none() {
            return Err(config(
                "budget",
                "DATABRICKS_RUNTIME_VERSION is set and no budget was given: the driver's cgroup limit is the machine's and the JVM already holds most of it, so set the budget explicitly, either as DiscoveryInput::explicit_budget or as the AMORU_BUDGET environment variable",
            ));
        }
        notes.push(
    ...
    let explicit_cpu = match (input.explicit_cpu, env.get("AMORU_CPU")) {
        (Some(cpu), _) => Some(cpu),
    ...
    let mut profile = match env.get("AMORU_HOST_PROFILE") {
        Some(text) => parse_profile(&text).inspect_err(|err| {
            tracing::error!(target: "discovery.profile_violation", error = %err, "AMORU_HOST_PROFILE is malformed");
        })?,
...
/// Run one probe, honouring `AMORU_TEST_FAIL_PROBE` in the crate's own tests only (section l).
#[cfg(test)]
fn run_probe(
    env: &dyn EnvSource,
    field: &'static str,
    ...
    let forced = env
        .get("AMORU_TEST_FAIL_PROBE")
        .is_some_and(|value| value.split(',').any(|name| name.trim() == field));
    if forced {
        return ProbeReport {
            available: false,
            note: Some(format!("AMORU_TEST_FAIL_PROBE forced `{field}` to fail")),
        };
    }
    probe()
}
...
fn resolve_staging_dir(
    ...
    if let Some(dir) = env.get("AMORU_SPILL_DIR") {
        candidates.push(("AMORU_SPILL_DIR".to_string(), PathBuf::from(dir)));
    }
...
mod tests {
    ...
    use amoru_kernel::{LimitSource, Sampler as SamplerTrait};
    ...
    fn ds_t1_precedence() {
        ...
        for (explicit, env_budget, expected, source) in cases {
            let mut pairs = vec![("AMORU_SPILL_DIR", spill.to_str().expect("utf-8"))];
            if let Some(value) = env_budget {
                pairs.push(("AMORU_BUDGET", value));
            }
        ...
        // CPU: explicit beats AMORU_CPU beats cpu.max beats the core count.
        let env = MapEnv::with(&[("AMORU_CPU", "2.5")]);
        let found = discover_with(
            &env,
            &roots,
            &DiscoveryInput {
        ...
        // Staging directory: explicit beats AMORU_SPILL_DIR beats the declared profile.
        let env = MapEnv::with(&[
    ...
    fn ds_t5_present_guarantee_enforced() {
        ...
        let env = MapEnv::with(&[
            ...
            ("AMORU_TEST_FAIL_PROBE", "io_uring"),
            ("AMORU_SPILL_DIR", spill.to_str().expect("utf-8")),
        ]);
        ...
        match err {
            amoru_kernel::AmoruError::Config { name, msg } => {
    ...
    fn ds_t11_databricks_budget_required() {
        ...
        match err {
            amoru_kernel::AmoruError::Config { name, msg } => {
                ...
                assert!(msg.contains("AMORU_BUDGET"), "{msg}");
        ...
        let env = MapEnv::with(&[("JAVA_HOME", "/opt/java"), ("AMORU_SPILL_DIR", spill_str)]);
    ...
    fn durable_staging_is_declared_not_probed() {
        ...
        let env = MapEnv::with(&[
            ("AMORU_HOST_PROFILE", "durable_staging=absent"),
            ("AMORU_SPILL_DIR", spill_str),
        ]);
        let found = discover_with(&env, &roots, &DiscoveryInput::default()).expect("discovery");
        ...
        let env = MapEnv::with(&[
            ("AMORU_HOST_PROFILE", "durable_staging=present"),
            ("AMORU_SPILL_DIR", "/amoru/definitely/absent"),
        ]);
        let err = discover_with(&env, &roots, &DiscoveryInput::default())
    ...
    fn the_caller_overrides_the_environment_profile() {
        ...
        let spill = staging(tmp.path());
        let env = MapEnv::with(&[
            ("AMORU_HOST_PROFILE", "gds=present,rdma=absent"),
            ("AMORU_SPILL_DIR", spill.to_str().expect("utf-8")),
        ]);
        ...
        let env = MapEnv::with(&[("AMORU_HOST_PROFILE", "gds=perhaps")]);
    ...
    fn an_unusable_staging_directory_leaves_no_disk_tier() {
        ...
        proc_fixture(tmp.path(), "0::/\n", 16 * 1024 * 1024);
        let roots = roots_at(tmp.path());
        let env = MapEnv::with(&[("AMORU_SPILL_DIR", "/amoru/definitely/absent")]);
        let found = discover_with(&env, &roots, &DiscoveryInput::default()).expect("discovery");
    ...
    /// `AMORU_SPILL_LIMIT` and malformed sizes are parsed here (section i, f.3).
    #[test]
    fn parses_the_remaining_environment_variables() {
        ...
        for bad in [
            ("AMORU_BUDGET", "lots"),
            ("AMORU_CPU", "none"),
            ("AMORU_SPILL_LIMIT", "8 GiB"),
        ] {
            let env = MapEnv::with(&[bad, ("AMORU_SPILL_DIR", spill_str)]);
            assert!(
