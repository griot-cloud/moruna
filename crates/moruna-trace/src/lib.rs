//! Moruna component 4, the trace writer and the run report.
//!
...
// Every fallible function here returns the contracts' `MorunaError` (contracts d.14), which is
...
/// The contracts' result type; every error in this crate is an `MorunaError` value.
pub type Result<T> = core::result::Result<T, moruna_kernel::MorunaError>;
...
/// (contracts d.1, b).
pub(crate) fn run_id_hex(id: moruna_kernel::RunId) -> String {
