...
fn arrow_err(op: &'static str) -> impl Fn(arrow::error::ArrowError) -> MorunaError {
    move |e| MorunaError::Io {
        op: "trace",
        target: op.to_string(),
        msg: e.to_string(),
    }
...
fn io_err(op: &'static str, target: &std::path::Path) -> impl Fn(std::io::Error) -> MorunaError {
...
impl TraceWriter {
    ...
    fn start_with_schema(cfg: TraceConfig, schema: SchemaRef) -> Result<Arc<TraceWriter>> {
        ...
        let handle = std::thread::Builder::new()
            .name("moruna-trace".to_string())
            .spawn(move || run_writer(thread_shared, rx, final_writer))
            .map_err(|e| MorunaError::Config {
                name: "trace.writer",
...
fn check_schema(schema: &SchemaRef) -> Result<()> {
    if TraceRecord::schema_hash() != TraceRecord::SCHEMA_HASH {
        return Err(MorunaError::Config {
            name: "trace.schema",
            msg: "moruna_kernel::TraceRecord::SCHEMA_HASH is not the digest of SCHEMA_FIELDS"
                .to_string(),
        });
    }
...
fn render_fields(schema: &SchemaRef) -> Result<String> {
    ...
    for (i, field) in schema.fields().iter().enumerate() {
        ...
        out.push(':');
        out.push_str(
            token_of(field.data_type()).ok_or_else(|| MorunaError::Config {
                name: "trace.schema",
                msg: format!(
...
fn drain_after_failure(shared: &Arc<Shared>, rx: &Receiver<Msg>) -> MorunaError {
...
impl WriterState {
    ...
    fn note(&mut self, e: MorunaError) {
...
mod tests {
    ...
    fn tr_t6_schema_hash() {
        let dir = std::env::temp_dir().join("moruna-trace-t6");
    ...
    fn a_failed_writer_drains_the_channel_and_counts_what_it_cannot_write() {
        ...
        let shared = Arc::new(Shared {
            ...
            overflow_failed: AtomicBool::new(false),
            finished: AtomicBool::new(false),
            overflow_path: std::env::temp_dir().join("moruna-trace-drain.arrow"),
            final_path: None,
            schema,
            memory_limit: 1024,
        });
    ...
    fn the_writer_carries_the_run_id() {
        let dir = std::env::temp_dir().join("moruna-trace-runid");
        let _ = std::fs::create_dir_all(&dir);
        let mut c = cfg(&dir);
