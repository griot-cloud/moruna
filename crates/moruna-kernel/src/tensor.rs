...
//! adds tier tracking and Moruna's contiguity checks. Handles are reference counted internally
...
/// the wrapper adds tier tracking and Moruna's contiguity checks.
...
impl ManagedTensor {
    ...
    pub fn from_dlpack(t: Dlpack) -> crate::Result<Self> {
        ...
        if version.major != dlpark::ffi::DLPACK_MAJOR_VERSION {
            ...
            return Err(MorunaError::Convert(ConvertError::Version {
                found: version.major,
        ...
        let shape = t
            .shape()
            .map_err(|e| MorunaError::Plan(format!("invalid DLPack shape: {e}")))?
            .to_vec();
        ...
        let strides = t
            .strides()
            .map_err(|e| MorunaError::Plan(format!("invalid DLPack strides: {e}")))?
            .map(<[i64]>::to_vec);
    ...
    pub fn from_buffer(
        ...
        let io = |msg: String| MorunaError::Io {
            op: "from_buffer",
            target: "buffer".into(),
            msg,
        };
    ...
    pub(crate) unsafe fn over_owner<C: Send + 'static>(
        ...
        let dlpack = builder
            ...
            .map_err(|e| MorunaError::Plan(format!("dlpack: {e}")))?;
        Ok(ManagedTensor {
            inner: Arc::new(Inner {
                dlpack: Some(dlpack),
                tier,
...
/// The DLPack dtype for an Moruna dtype.
...
/// The Moruna dtype for a DLPack dtype; `None` when there is no one-to-one match.
...
fn device_of(tier: Tier) -> crate::Result<DLDevice> {
    match tier {
        ...
        Tier::Disk(_) => Err(MorunaError::Staging("tensor is not resident (Disk)".into())),
...
fn tier_of(device: DLDevice) -> crate::Result<Tier> {
    match device.device_type {
        ...
        DLDeviceType::CUDA => u8::try_from(device.device_id)
            ...
            .map_err(|_| {
                MorunaError::Plan(format!(
                    "DLPack device id {} out of range",
                    device.device_id
                ))
            }),
        other => Err(MorunaError::Plan(format!(
