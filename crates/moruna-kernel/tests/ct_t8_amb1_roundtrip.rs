...
/// The reference writer: a whole `MRB1` record as bytes, header, padding, payload, tail padding.
...
fn is_amb1_error(err: &MorunaError) -> bool {
    matches!(err, MorunaError::Io { op: "mrb1", .. })
}
