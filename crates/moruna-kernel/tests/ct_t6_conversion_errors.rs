...
fn ct_t6_conversion_errors() {
    ...
    assert!(
        matches!(err, MorunaError::Convert(ConvertError::HasNulls(ref name)) if name == "c"),
    ...
    assert!(matches!(err, MorunaError::Plan(_)));
    ...
    assert!(
        matches!(err, MorunaError::Convert(ConvertError::NotNumeric(ref name)) if name == "b"),
    ...
    assert!(
        matches!(err, MorunaError::Convert(ConvertError::NotContiguous)),
    ...
    assert!(
        matches!(err, MorunaError::Convert(ConvertError::Rank(3))),
    ...
    assert!(
        matches!(err, MorunaError::Convert(ConvertError::NotNumeric(_))),
        "got {err}"
    );
    ...
    assert!(
        matches!(err, MorunaError::Convert(ConvertError::MixedDTypes)),
        "got {err}"
    );
    ...
    let device = common::tagged_buffer(16, Tier::Device(moruna_kernel::DeviceId(0)));
