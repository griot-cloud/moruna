...
impl Sink for DefaultSink {
    ...
    fn write(&self, _seq: Seq, _payload: Payload) -> BoxFuture<'_, moruna_kernel::Result<()>> {
        Box::pin(async { Ok(()) })
    }
...
fn ct_t15_resume_defaults() {
    ...
    assert!(
        matches!(err, MorunaError::Resume(_)),
    ...
    impl Source for DefaultSource {
        ...
        fn read(
            &self,
            _split: &moruna_kernel::Split,
            _rows: Option<moruna_kernel::RowRange>,
            _alloc: &dyn moruna_kernel::Allocator,
            _tier: moruna_kernel::Tier,
        ) -> BoxFuture<'_, moruna_kernel::Result<Payload>> {
            Box::pin(async {
                Err(MorunaError::Source {
                    split: 0,
                    msg: "no splits".into(),
                })
            })
