...
use amoru_kernel::{
    AmoruError, CheckpointExtras, Fingerprint, NodeId, Origin, PayloadKind, ResumePoint,
...
fn resume(path: &Path, what: impl std::fmt::Display) -> AmoruError {
    AmoruError::Resume(format!("{}: {what}", path.display()))
}

/// Read a manifest from disk and parse it (f.13).
pub fn read_manifest(path: &Path) -> amoru_kernel::Result<Manifest> {
...
impl PlacementEngine {
    ...
    pub(crate) fn write_manifest(
        ...
        let text = serde_json::to_string_pretty(&manifest).map_err(|e| AmoruError::Io {
            op: "serialise",
        ...
        let io = |op: &'static str, target: &Path, e: std::io::Error| AmoruError::Io {
            op,
