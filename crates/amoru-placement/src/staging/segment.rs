...
impl RecordHeader {
    ...
    pub fn write(&self, out: &mut [u8]) -> amoru_kernel::Result<()> {
    ...
    pub fn read(buf: &[u8]) -> amoru_kernel::Result<RecordHeader> {
        ...
        if buf[0..8] != MAGIC {
            return Err(AmoruError::Staging("record magic is not AMORUSEG".into()));
        }
        ...
        let Some(codec) = StagingCodec::from_code(buf[19]) else {
            return Err(AmoruError::Unsupported("codec"));
        };
        let mut eight = [0u8; 8];
...
impl PlacementEngine {
    ...
    /// `staging_dir/amoru-<run_id>/seg-<segment:06>.seg` (e.3).
    ...
    pub(crate) fn reserve_record(
        ...
        stage: StageId,
        need: u64,
    ) -> amoru_kernel::Result<(u32, PathBuf, u64)> {
    ...
    fn open_segment(&self, stage: StageId, need: u64) -> amoru_kernel::Result<u32> {
        ...
        let file = std::fs::File::create(&path).map_err(|e| AmoruError::Io {
            op: "create",
