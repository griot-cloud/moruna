...
//! (the record header and the Arrow IPC framing or the `AMB1` header), which are not payload
//! (G-I2, PL-I4).

use std::sync::Arc;

use amoru_kernel::arrow::buffer::Buffer as ArrowBuffer;
use amoru_kernel::{Allocator, AmoruError, Buffer, BufferView, ManagedTensor, Tier, amb1, ipc};
...
impl Piece {
    ...
    pub fn view(&self, alloc: &dyn Allocator) -> amoru_kernel::Result<BufferView> {
        match &self.src {
...
pub struct RecordDraft {
    /// The metadata page: the IPC framing for a table, the `AMB1` header for a tensor.
    pub meta: Buffer,
...
pub fn draft(
    ...
    page: u64,
    alloc: &dyn Allocator,
) -> amoru_kernel::Result<RecordDraft> {
    match payload {
        ...
        PayloadRef::Tensor(tensor) => {
            if !tensor.is_contiguous() {
                return Err(AmoruError::Convert(
                    amoru_kernel::ConvertError::NotContiguous,
                ));
            }
            let shape = tensor.shape().to_vec();
            let data_offset = amb1::Header::data_offset_for(shape.len(), page)?;
...
/// finding.
fn arena_backed(buffer: ArrowBuffer, alloc: &dyn Allocator) -> amoru_kernel::Result<PieceSrc> {
