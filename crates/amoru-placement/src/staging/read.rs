...
use amoru_kernel::{AmoruError, Buffer, Payload, PayloadKind, amb1, ipc};
...
pub fn decode_record(buf: Buffer, page: u64) -> amoru_kernel::Result<Record> {
    let page_usize = usize::try_from(page)
        .map_err(|_| AmoruError::Staging(format!("page size {page} does not fit this platform")))?;
    ...
    let payload = match header.kind {
        ...
        PayloadKind::Tensor => {
            let parsed = amb1::Header::read(&buf[page_usize..])?;
            let (dtype, shape, data_offset) = (parsed.dtype, parsed.shape, parsed.data_offset);
            let tensor =
                amoru_kernel::ManagedTensor::from_buffer(buf, page + data_offset, dtype, shape)?;
