//! Amoru component 9, the placement engine: the queues between stages, the tier each
...
// d.4, e.7).
#![forbid(unsafe_code)]
// `AmoruError` carries a morsel's features so a diagnostic needs no debugger (CT-I10), which
...
use amoru_kernel::{
    Allocator, AmoruError, CheckpointExtras, DeviceId, Fingerprint, Locality, Morsel, NodeId,
...
impl PlacementEngine {
    ...
    pub fn new(
    ...
    ) -> amoru_kernel::Result<Arc<PlacementEngine>> {
        if cfg.stages == 0 {
        ...
        let run_dir = match &cfg.staging_dir {
            Some(dir) => {
                let path = dir.join(format!("amoru-{}", cfg.run_id.to_hex()));
                std::fs::create_dir_all(&path).map_err(|e| AmoruError::Io {
                    op: "create_dir_all",
    ...
    pub fn find_manifest(
        staging_dir: &Path,
        run_id: Option<RunId>,
    ) -> amoru_kernel::Result<Option<PathBuf>> {
    ...
    pub fn read_manifest_header(path: &Path) -> amoru_kernel::Result<ManifestHeader> {
    ...
    fn check_stage(&self, stage: StageId) -> amoru_kernel::Result<usize> {
...
impl Placement for PlacementEngine {
    fn push(&self, stage: StageId, morsel: Morsel) -> amoru_kernel::Result<()> {
    ...
    fn pop(
        ...
        want: PayloadSpec,
        locality: Locality,
    ) -> amoru_kernel::Result<Option<Morsel>> {
        let index = self.check_stage(stage)?;
    ...
    fn pop_blocking(
        ...
        want: PayloadSpec,
        locality: Locality,
    ) -> amoru_kernel::Result<Option<(Morsel, u64)>> {
        let index = self.check_stage(stage)?;
    ...
    fn evicted(&self, stage: StageId) -> Vec<(Seq, amoru_kernel::Origin)> {
        match self.check_stage(stage) {
    ...
    fn replace(&self, stage: StageId, morsel: Morsel) -> amoru_kernel::Result<()> {
        let index = self.check_stage(stage)?;
    ...
    fn checkpoint(&self, extras: &CheckpointExtras) -> amoru_kernel::Result<PathBuf> {
    ...
    ) -> amoru_kernel::Result<ResumePoint> {
