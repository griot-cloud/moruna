...
use amoru_kernel::{AmoruError, Locality, Placement, Reactor, TIER_COUNT, TierKind};
...
fn pl_t22_the_source_state_of_every_move_is_where_its_bytes_were() {
    ...
    use amoru_kernel::{NodeId, RemoteRef, SegmentRef, Tier};
    use amoru_placement::shutdown::is_settled;
    let seg = SegmentRef {
        segment: 3,
        offset: 4096,
        len: 128,
    };
    let promoting = State::Promoting(Tier::Host, Tier::Device(amoru_kernel::DeviceId(0)));
