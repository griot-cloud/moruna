...
use amoru_kernel::{
    AmoruError, CheckpointExtras, Fingerprint, Locality, Placement, ResumePolicy, RunId,
    SourceCursor, TierKind,
};
use amoru_placement::manifest::Manifest;
use amoru_placement::state::State;
use amoru_placement::{PlacementConfig, PlacementEngine};
...
fn refuse(
    ...
    match engine.restore(&edited, &common::plan(), &cfg.fingerprints) {
        ...
        Err(AmoruError::Resume(message)) => message,
...
fn pl_t16_find_manifest_takes_the_newest() {
    ...
    let older = scratch.path().join("amoru-".to_string() + &"aa".repeat(16));
    let newer = scratch.path().join("amoru-".to_string() + &"bb".repeat(16));
...
fn pl_t16_the_manifest_names_every_policy_and_both_payload_kinds() {
    ...
    let path = engine
        ...
        .expect("checkpoint");
    let manifest = amoru_placement::manifest::read_manifest(&path).expect("manifest");
    ...
    assert!(amoru_placement::manifest::read_manifest(&broken).is_err());
    ...
    assert_eq!(
        amoru_placement::manifest::plan_digest(&common::plan()),
        amoru_placement::manifest::plan_digest(&estimated),
    ...
    assert_ne!(
        amoru_placement::manifest::plan_digest(&common::plan()),
        amoru_placement::manifest::plan_digest(&renumbered),
