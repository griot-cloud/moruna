...
use amoru_kernel::{AmoruError, Locality, Placement, TierKind};
...
fn pl_t14_one_failure_is_retried_identically() {
    ...
    assert!(
        matches!(
            engine.head_state(0),
            Some((0, State::Resident(amoru_kernel::Tier::Device(_))))
        ),
...
fn pl_t14_two_failures_surface_through_pop() {
    ...
    assert!(matches!(error, AmoruError::Staging(_)), "got {text}");
...
fn pl_t14_a_failed_write_leaves_the_bytes_resident() {
    ...
    let before = alloc.in_use(amoru_kernel::Tier::Host);
    for seq in 0..2u64 {
        engine
    ...
    assert!(
        alloc.in_use(amoru_kernel::Tier::Host) > before,
