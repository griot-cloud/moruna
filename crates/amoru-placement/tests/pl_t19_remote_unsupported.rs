...
use amoru_kernel::{
    AmoruError, DeviceId, LOCAL_NODE, Locality, Morsel, NodeId, Payload, Placement, RemoteRef,
    SegmentRef, StagingCodec, Tier, TierKind, TierPref,
};
use amoru_placement::plan::one_tier_down;
use amoru_placement::staging::segment::RecordHeader;
use amoru_placement::state::{State, satisfies};
...
fn pl_t19_a_remote_head_cannot_be_popped() {
    ...
    assert!(!satisfies(
        ...
        &amoru_kernel::PayloadSpec {
            kind: amoru_kernel::PayloadKind::Either,
            tier: TierPref::Any,
        }
    ));
    assert_eq!(
...
fn pl_t19_an_unknown_codec_byte_is_refused() {
    ...
    match RecordHeader::read(&page) {
        Err(AmoruError::Unsupported("codec")) => {}
