...
//! `amoru_kernel::ipc::decode`, which is the reader the format is written for. Reported as a
//! finding against 09 e.3.

mod common;

use amoru_kernel::arrow::buffer::Buffer as ArrowBuffer;
use amoru_kernel::{Locality, Placement, StagingCodec, TierKind, amb1, ipc};
...
fn parse(page: &[u8]) -> Header {
    assert_eq!(&page[0..8], b"AMORUSEG", "magic at offset 0");
...
fn run_dir(scratch: &common::Scratch) -> std::path::PathBuf {
    scratch.path().join(format!("amoru-{}", "07".repeat(16)))
}
    ...
    for number in &numbers {
        ...
        while (at + page) as usize <= file.len() && file[at as usize..at as usize + 8] != [0u8; 8] {
            ...
            match header.kind {
                ...
                1 => {
                    let parsed = amb1::Header::read(body).expect("the AMB1 body parses");
...
fn pl_t10_a_record_that_is_not_one_is_refused() {
    ...
    use amoru_kernel::PayloadKind;
    use amoru_placement::staging::read::decode_record;
    use amoru_placement::staging::segment::RecordHeader;
    let alloc = FakeAllocator::new();
    let page = common::PAGE as u64;
    ...
    assert!(
        ...
        "a body that is not an AMB1 record is refused"
    );
}
