...
impl TraceView {
    ...
    pub fn to_ipc_file(&self, path: &Path) -> Result<()> {
        if let Some(dir) = path.parent().filter(|d| !d.as_os_str().is_empty()) {
            std::fs::create_dir_all(dir).map_err(|e| AmoruError::Io {
                op: "trace_dir",
                target: dir.display().to_string(),
                msg: e.to_string(),
            })?;
        }
        let file = File::create(path).map_err(|e| AmoruError::Io {
            op: "trace_create",
        ...
        for batch in self.batches() {
            w.write(&batch).map_err(|e| AmoruError::Io {
                op: "trace_ipc",
                target: path.display().to_string(),
                msg: e.to_string(),
            })?;
        }
        w.finish().map_err(|e| AmoruError::Io {
            op: "trace_ipc",
            target: path.display().to_string(),
            msg: e.to_string(),
        })
