...
fn tr_t10_layout_stays_inside_forty_lines() {
    ...
    let mut m = meta(amoru_trace::ExitReason::Terminated {
        diagnostic: "budget: morsel 3 stage 7 footprint 9 exceeds budget 8".to_string(),
    });
    m.resumed = true;
    m.manifest = Some(std::path::PathBuf::from("/tmp/amoru/run/manifest.json"));
    ...
    assert!(
        shown.contains("manifest: /tmp/amoru/run/manifest.json"),
        "{shown}"
    );
    ...
    let plain = RunReport::compute(&view, &limits(), &meta(amoru_trace::ExitReason::Completed));
