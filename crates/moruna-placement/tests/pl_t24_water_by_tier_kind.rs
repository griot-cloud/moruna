...
use moruna_kernel::{DeviceId, Locality, Placement, Tier, TierKind};
...
fn pl_t24_the_engine_refuses_a_configuration_it_cannot_honour() {
    use moruna_kernel::{Allocator, MorunaError, Reactor};
    use std::sync::Arc;
    ...
    none.stages = 0;
    match moruna_placement::PlacementEngine::new(none, allocator.clone(), io.clone()) {
        Err(MorunaError::Config { name, .. }) => assert_eq!(name, "stages"),
    ...
    match moruna_placement::PlacementEngine::new(pageless, allocator.clone(), io.clone()) {
        Err(MorunaError::Config { name, .. }) => assert_eq!(name, "page.bytes"),
        Err(other) => panic!("expected Config, got {other}"),
        Ok(_) => panic!("expected Config, got an engine"),
    }
    let mut segmentless = base.clone();
    segmentless.segment_bytes = 0;
    match moruna_placement::PlacementEngine::new(segmentless, allocator, io) {
        Err(MorunaError::Config { name, .. }) => assert_eq!(name, "staging.segment_bytes"),
        Err(other) => panic!("expected Config, got {other}"),
        Ok(_) => panic!("expected Config, got an engine"),
    }
    assert_eq!(
        moruna_placement::PlacementConfig::default().stages,
        1,
        "the default configuration is one queue and no staging directory"
    );
    assert!(
        moruna_placement::PlacementConfig::default()
            .staging_dir
            .is_none()
    );
}
