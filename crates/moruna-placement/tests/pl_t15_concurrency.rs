...
fn pl_t15_concurrency() {
    ...
    // figure that keeps the gate honest and quick; `MORUNA_PL_T15_ENTRIES` raises it for a
    // soak run. Reported as a deviation from 09 k (PL-T15).
    let entries: u64 = match std::env::var("MORUNA_PL_T15_ENTRIES") {
    ...
    assert_eq!(detail.reservations, [0; moruna_kernel::TIER_COUNT]);
    assert!(
