...
use moruna_kernel::{CheckpointExtras, Locality, Placement, TierKind};
use moruna_placement::state::State;
use moruna_testkit::{FakeAllocator, FakeReactor};

fn drive(
    engine: &moruna_placement::PlacementEngine,
    alloc: &FakeAllocator,
    count: u64,
    rows: usize,
) {
...
fn pl_t7_unlinked_at_once_without_checkpoints() {
        ...
        .expect("pop_blocking")
    {
        ...
        for (seq, state) in engine.entry_states(0) {
            if let State::OnDisk(seg) = &state {
                let path = scratch
                    .path()
                    .join(format!("moruna-{}", "07".repeat(16)))
    ...
    let run_dir = scratch.path().join(format!("moruna-{}", "07".repeat(16)));
...
fn pl_t7_a_referenced_segment_survives_until_the_next_manifest() {
    ...
    let path = engine
        ...
        .expect("checkpoint");
    let referenced = moruna_placement::manifest::read_manifest(&path).expect("manifest");
    assert!(
